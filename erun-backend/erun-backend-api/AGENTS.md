...
- A `READY` review becomes `MERGE` only when it is advanced as the next item for that target branch. Promoting a review to `MERGE` dispatches the merge queue's own gate build of the prospective merge onto the *current* target — the property that catches two reviews that are each green alone but broken together before the target branch moves.
...
- `MERGED` is never a caller-reported status. `PATCH /v1/reviews/{review_id}/status` refuses both `MERGE` and `MERGED`; the merge queue's own gate result (`internal/service/mergequeue.go`, `internal/mergeexec`, `internal/provision/merges.go`) is the only path that writes `MERGED`, and only once it has actually merged the review's source onto its target and gated the result with a real `erun build`. `builds.kind = 'GATE'` marks that build; it carries no version because the gate publishes nothing.
...
- `ReadAll`/`WriteAll` at enrolment is otherwise **not** the default. `UserRepository.Create` (the shared enrolment path behind `POST /v1/users`, `POST /v1/identity/users`, and invite acceptance) grants only the roles named in a caller-supplied `roleIds`, and grants nothing when `roleIds` is empty — an enrolled user who can sign in and do nothing until someone with a grant-capable role grants one. The tenant's first user is the only enrolment-path exception (mirrors the two bootstrap bullets above): granting a role is itself permission-gated, so a first user enrolled with nothing could never be granted anything.
...
- The merge queue is what makes `MERGED` mean something happened, not something a caller asserted: promoting a review to `MERGE` (`ReviewService.AdvanceMergeQueue`) dispatches `internal/mergeexec` to fetch the review's target and source, build the prospective squash merge locally, gate it with a real `erun build`, and push only on green. `MERGED` and its `builds.kind = 'GATE'` build are written by `internal/service/mergequeue.go` from that outcome, never by `PATCH .../status`.
- The gate is `erun build`, never `erun release`. The gate publishes nothing — releasing stays exactly where #1030 put it, after the merge lands, triggered off the merge commit the gate produced.
- `MergeQueueService` depends on the raw `ReviewRepository`/`BuildRepository`, not on `ReviewService`/`BuildService`, and applies the `MERGE` → `MERGED`/`FAILED` transition itself. This is a deliberate, narrow duplication of a few lines of `ReviewService`'s transition logic: `ReviewService` must stay free of any dependency on the merge executor, because the executor (via `BuildService`, which needs `ReviewService` for `MarkBuildResult`) would otherwise depend back on it, and Go composition cannot wire a cycle. `BuildService.Create` and `routes.advanceMergeQueue` each dispatch the merge queue for whichever review `AdvanceMergeQueue` actually promotes — not necessarily the review the current request is about, since the promoted review is always the current queue head for that target branch.
...
- The durable workflow (`internal/provision/merges.go`) is keyed by `reviewID + lastReadyBuildID`, which is stable across a control-plane restart for one attempt and distinct for every attempt driven by a new build. The one gap this does not cover: the missed-merge-window requeue path (`requeueMergingReview`) re-promotes the same review with the same `LastReadyBuildID` and so reuses the same workflow/Job key — an accepted, rare, operator-triggered edge case, not the normal retry path.
...
- **`provision.MergeConfig`'s six fields are wired end to end (`cmd/eapi/main.go`'s `ERUN_MERGE_*` env vars, the `erun-devops` chart's `api.mergeQueue` block), but the gate cannot actually succeed against a real cluster yet.** `internal/mergeexec/job.go`'s Job mounts only the agent environment's `HomeClaim`/`WorkspaceClaim` — no docker socket, no dind sidecar, no `DOCKER_HOST` — so `erun build` has no daemon to connect to and fails immediately, verified against a real disposable `batch/v1.Job` in a real namespace on real disposable branches. `internal/releaseexec/job.go` has the identical gap. Fixing this needs a design decision (the environment's warm BuildKit cache lives behind its own long-running dind sidecar's `emptyDir` socket, which a separate Job pod cannot reach, and a second dind against the same `/var/lib/docker` PVC risks corrupting it) — see #1463. Do not enable `api.mergeQueue`/`api.releaseQueue` against a real control plane until that is resolved: today it would only turn "recorded, ungated" into "recorded, gated, and always FAILED."
...
- The merge queue has the same three-tier gate shape: `internal/mergeexec/job_test.go` runs the exact Job script against real local git repositories with no cluster or database at all (a stub `erun` stands in for the toolchain — the property under test is the merge, not the build binary) and is the test that proves two reviews can be green alone but broken together; `ERUN_E2E_REVIEWS_DATABASE_URL` runs the `builds.kind`/version/failure_detail contract and tenant isolation against a migrated PostgreSQL (`internal/repository/reviews_e2e_test.go`); `ERUN_E2E_MERGE_QUEUE=1` plus a cluster, a migrated Postgres, and a disposable checkout runs the whole pipeline (`merge_queue_e2e_test.go`) — unlike the release gate there is no dry-run mode, since proving the gate actually pushes is the point, so its target and source branches must be disposable, never a real integration branch.
