...
- `GET /v1/gate-runs`/`GET /v1/gate-runs/{gate_run_id}` are marked internal in `route_audit.go`'s `InternalAPIRoutes` as a **deliberate, temporary** scope decision for #1931's CLI-first delivery, not a claim that they need no operator surface forever — see #1932 for the follow-up console/desktop surface, and remove the entries once that surface exists. `POST /v1/gate-runs` and `PATCH /v1/gate-runs/{gate_run_id}` are internal permanently: the environment driving the gate reports its own attempt, the same self-report shape as `POST /v1/reviews/{review_id}/builds`.

### GitHub branch protection cannot tell the queue's push from a bypass (#1912, analysis recorded, not yet implemented)
...
- **A bypass is structurally unavoidable for the `pull_request` rule as long as the queue pushes raw commits instead of merging through the GitHub API**, so "make every bypass reconcilable after the fact" (the issue's third option) is not a competing design — it is a complement worth keeping regardless of which identity holds the bypass grant: cross-reference each bypass log entry's commit against a recorded, successful `GATE` build for that exact commit (the ingredients already exist — `record-build --gate`, `acceptMerged`'s server-side verification, `gitverify`).
- **Not done by this analysis, and deliberately left as an explicit ops decision rather than something a code PR should decide unilaterally:** provisioning the dedicated queue identity and deciding whether release shares it; moving the environment(s) that run `erun-merge-queue-drive`/`erun release` to authenticate as it (verify with `git push --dry-run` first, the same check already used to confirm the current shared credential works); removing or narrowing the existing broad `bypass_actors` entries on ruleset `11081432` and adding the new identity as the sole `always` bypass actor; and, only after a producer is proven, deciding separately whether to add a `required_status_checks` rule and what it should require. None of these are performed here.

## Validation

- Run `go test ./...` from this module after Go changes.
- The queue gates are opt-in and live in this module's root package. `ERUN_E2E_RELEASE_DATABASE_URL` runs the release queue's SQL contracts (`ClaimNext`/`RecordOutcome`/`ExpireStale`/idempotency) against a migrated PostgreSQL (`release_queue_sql_e2e_test.go`); `ERUN_E2E_REVIEWS_DATABASE_URL` runs the `builds.kind`/version/failure_detail contract and tenant isolation against a migrated PostgreSQL (`internal/repository/reviews_e2e_test.go`); `ERUN_E2E_MERGE_DATABASE_URL` and `ERUN_E2E_RELEASE_DATABASE_URL` each additionally run an HTTP-level end-to-end gate (`merge_queue_e2e_test.go`, `release_queue_e2e_test.go`) — no cluster, no DBOS, since neither queue dispatches a Job anymore. The merge queue's gate drives a real local (`file://`) git remote through the exact fetch/merge/push steps an environment performs, and is the test that proves a merge built against a target tip that is no longer current gets refused even once it is genuinely pushed to the branch (a force-push standing in for a buggy or malicious reporter, since a well-behaved fast-forward-only push cannot manufacture that state).
- `internal/gitverify` has its own unit tests against real local git repositories (`verifier_test.go`) — no cluster or database, the same style `internal/mergeexec/job_test.go` used before this package replaced it.
...
- The mandatory unit-level refusal tests for `acceptMerged`'s three verification conditions live in `internal/service/reviews_test.go` (`TestAcceptMergedRefusesWhenCommitIsNotOnTheTargetBranch`, `TestAcceptMergedRefusesWhenParentDoesNotMatchTheGatedTargetTip`, `TestAcceptMergedRefusesWithNoSuccessfulGateBuildRecorded`), each driven by a fake `MergeVerifier` so the condition under test is isolated from the other two.
