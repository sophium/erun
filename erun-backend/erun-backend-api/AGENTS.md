...
- **The backend drives every automation through Go SDKs, never by executing external binaries.** The API process (and the DBOS durable workflows it runs) must not shell out to `kubectl`, `helm`, `aws`, `docker`, or any other CLI — talk to Kubernetes via `client-go`, to Helm via `helm.sh/helm/v3`, to AWS via `aws-sdk-go-v2`, etc. The `erun-common` deploy/provision helpers that shell CLIs (`RunHelmDeploy`/`DeployHelmChart`/`EnsureKubernetesNamespace`, the `aws`/`kubectl` runner closures) are the CLI/desktop transport's path; the hosted backend reimplements the same outcomes with SDKs behind its `internal/` packages, with no subprocess: `internal/deploy` drives Kubernetes + Helm via `client-go` + `helm.sh/helm/v3` (see `internal/deploy/k8sclient.go`, `helm.go`); `internal/provision` drives AWS via `aws-sdk-go-v2` (see `internal/provision/awssdk.go`, which implements `eruncommon.InitCloudContext`'s injected `RunAWS` seam against the SDK). The `erun-backend-api` runtime image is therefore a thin alpine that ships only the `eapi` binary + `ca-certificates` — no `aws`/`kubectl`/`helm`. Do not reintroduce subprocess execution, do not add new CLI shell-outs, and do not add those CLIs to the image. This invariant is enforced: `TestBackendRunsNoExternalBinaries` (`noexec_test.go`) fails the build if any production file imports `os/exec` or calls erun-common's CLI-shelling helpers.
- Keep the SDK dependency surface (`helm.sh/helm/v3` + the `k8s.io` quartet, `aws-sdk-go-v2`) in `erun-backend-api` only. Never add a Kubernetes/Helm/cloud-SDK import to `erun-common` (its `AGENTS.md` forbids it) — the SDK-driven execution lives behind the backend's `internal/` packages.
