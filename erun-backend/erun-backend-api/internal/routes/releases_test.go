...
func (d *stubDispatcher) Dispatch(_ context.Context, tenantName string) (int, error) {
	d.tenants = append(d.tenants, tenantName)
	return 0, d.err
}
...
func (t stubTenants) Current(context.Context) (model.Tenant, error) { return t.tenant, t.err }

func releaseRoutes(queue *stubReleaseQueue, releases *stubReleaseRepository, dispatcher ReleaseDispatcher) ReleaseRoutes {
	return ReleaseRoutes{
		releases:   releases,
		queue:      queue,
		dispatcher: dispatcher,
		tenants:    stubTenants{tenant: model.Tenant{Name: "acme"}},
	}
}
...
func TestCreateReleaseQueuesTheCommit(t *testing.T) {
	...
	rec := postRelease(t, releaseRoutes(queue, &stubReleaseRepository{}, dispatcher),
	...
	// The trigger drains the queue itself, so a release does not wait for the
	// next unrelated request to start.
	if len(dispatcher.tenants) != 1 || dispatcher.tenants[0] != "acme" {
		t.Fatalf("dispatched %v, want one dispatch for the caller's tenant", dispatcher.tenants)
	}
}
...
func TestCreateReleaseAnswers200ForAnAlreadyReleasedCommit(t *testing.T) {
	...
	rec := postRelease(t, releaseRoutes(queue, &stubReleaseRepository{}, &stubDispatcher{}),
...
func TestCreateReleaseRejectsAnInvalidRequest(t *testing.T) {
	...
	rec := postRelease(t, releaseRoutes(queue, &stubReleaseRepository{}, &stubDispatcher{}), `{"commitId":"commit-a"}`)
	if rec.Code != http.StatusBadRequest {
		t.Fatalf("status = %d, want 400", rec.Code)
	}
}

// TestCreateReleaseSurvivesADispatchFailure: the request is recorded, and the
// next trigger or the next finishing release picks it up, so a dispatch problem
// must not lose the trigger.
func TestCreateReleaseSurvivesADispatchFailure(t *testing.T) {
	queue := &stubReleaseQueue{result: service.EnqueueResult{Created: true, Release: model.Release{ReleaseID: "rel-1"}}}
	rec := postRelease(t, releaseRoutes(queue, &stubReleaseRepository{}, &stubDispatcher{err: errors.New("cluster unreachable")}),
		`{"targetBranch":"main","commitId":"commit-a"}`)
	if rec.Code != http.StatusCreated {
		t.Fatalf("status = %d, want the trigger to still be recorded as 201", rec.Code)
	}
}

func TestCreateReleaseWithoutADispatcherStillRecords(t *testing.T) {
	queue := &stubReleaseQueue{result: service.EnqueueResult{Created: true, Release: model.Release{ReleaseID: "rel-1"}}}
	rec := postRelease(t, releaseRoutes(queue, &stubReleaseRepository{}, nil), `{"targetBranch":"main","commitId":"commit-a"}`)
	if rec.Code != http.StatusCreated {
...
func TestListReviewReleasesScopesToTheReview(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	releaseRoutes(&stubReleaseQueue{}, releases, nil).listReviewReleases(rec, req)
...
func TestListReleasesPassesItsFilters(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	releaseRoutes(&stubReleaseQueue{}, releases, nil).listReleases(rec, req)
