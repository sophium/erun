CREATE TABLE builds (
  ...
  failure_detail TEXT,
  -- The bounded per-build profile (root AGENTS.md #2274) a caller self-reports
  -- alongside its outcome -- a duration/CPU/throttle/IO summary plus the
  -- top costliest steps, never the full step tree. NULL when the caller
  -- collected none (an older erun version, or a build outside a runtime pod).
  profile JSONB,
