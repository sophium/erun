...
## Retention For Review Comments And Releases (#1970, design recorded — not yet implemented)

- **Split out of #1968's six-table sweep, grouped together because they share an answer that `reviews` — the table both hang off — does not.** `reviews` itself (#1971) has six inbound `RESTRICT` foreign keys and a documented "Branch history stays unbounded" property that may be a deliberate product promise, the same obligation-vs-performance question #1959 had to resolve for audit trails. `comments` and `releases` carry neither complication: each is a leaf with nothing referencing it back, and neither has any stated permanence guarantee anywhere in this repo. That is what makes them answerable now, in the same pass, while `reviews` needs its own harder design.
