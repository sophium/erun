...
- `src/App.tsx` — wires the token source → `fetchConfig`, rendering one of `shell/PreShellScreens.tsx`'s pre-sign-in screens or `shell/AppShell.tsx` once a token has produced a rendered config.
...
- Express "may I?" through the shared capability shape rather than a console-local predicate. The shared kit landed (#1211) and the backend already reports `capabilities` on `GET /v1/whoami` (#1210), but the console has no store or capability client yet — that lands with the console's data layer (Redux + `httpBaseQuery`, tracked separately from the app shell in #1207). Until then the nav and panels still gate the identity sections on `tenant.type === 'OPERATIONS'` (`shell/sections.ts`), matching the backend's own restriction; the first surface to need finer-grained gating adds the capability shape to the shared kit, not to a component.
...
- Dark mode is the class-based `.dark` toggle `erun-kit/theme.css` already defines the token scale for (`shell/theme.ts`/`shell/useTheme.ts`), persisted to `localStorage` and defaulted from `prefers-color-scheme`. `erun-ui/frontend` ships the same token scale but has no toggle wired to it yet; the console's mechanism is the one to reuse there since it's the same `.dark` class contract.

## Professional UX

- The console is under the same UX contract as the desktop. Apply `erun-ui/AGENTS.md` §§ "Professional UX" and "UX Impact Review Checklist" in full — same Nielsen Norman/WCAG/Material references, same pre-implementation note (user task, control choice, states, principles applied), same checklist answers recorded in the PR description. Nothing in those sections is desktop-specific; read "the app" as "whichever of the desktop or the console the change touches."
