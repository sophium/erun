...
**MCP access tokens are implemented** (`src/mcp/`, issue #686): the console mints a short-lived per-env MCP bearer via `POST /v1/environments/{id}/mcp-token` (the backend signs it; the deploy injected the matching public key into the env, so the edge verifies it) and surfaces it for the operator to present to that env's MCP edge. Verified by `MCPAccessPanel.test.tsx` against a mocked `fetch`.

Still **deliberately not implemented** (flagged in source):

- **Calling the env's per-env MCP from the console.** The edge lives at `mcp.<tenant>-<env>.services.<base-domain>` behind the per-env auth edge; RCE-sensitive (`raw` can `kubectl exec`). The console can already mint the token (above); *driving the tools* over that public edge needs the live edge (Cloudflare/DNS) to verify, so the in-console MCP tool UI is the infra-gated remainder.

When you implement it, add the verification (a live-env test, not a mock) in the same PR.
...
- `src/auth/` — the OIDC sign-in (`auth.ts`, `auth.test.ts`).
- `src/mcp/` — the per-env MCP-access surface (issue #686): `controller.ts` is the thin one-shot mint controller (`useMCPTokenController`, calls `mintMCPToken` = `POST /v1/environments/{id}/mcp-token`), and `MCPAccessPanel.tsx` is the render layer (per **deployed** runtime env: a "Get MCP access token" action that mints + surfaces the token + its per-env audience). `MCPAccessPanel.test.tsx` exercises the mint + the error path against a mocked `fetch`.
