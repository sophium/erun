...
// requestMcpToken mints a per-env MCP bearer token the console presents to the
// env's erun-mcp edge. The backend signs it, so the caller needs no key. A 501
// surfaces when the backend has no MCP signing key configured.
export async function requestMcpToken(token: string, environmentId: string): Promise<McpToken> {
  const response = await authedFetch(
  ...
  if (!response.ok) {
    throw new ConfigFetchError(
      `mcp token request failed (${String(response.status)})`,
      response.status,
    );
  }
  const body: unknown = await response.json();
