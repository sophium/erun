...
- Tests run inside the build, per the delivery-pipeline convention (docs `/pipeline`, "How you set it up — and where tests run"): the `erun-devops` Dockerfile has a `test` stage that runs `make integration-test`, and the builder stage depends on its marker, so a failing gate fails the build and no image is tagged. Pin test stages to `--platform=$BUILDPLATFORM` so the suite runs natively once per build instead of once per target arch under emulation. PR-time CI (`.github/workflows/build.yml`) runs `erun build` itself, so the in-build gate is the same gate everywhere: every built or released image is a tested build.
