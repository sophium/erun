...
- Charts deploy in dependency order regardless of `--components` input order. The order can be configured per project in `.erun/config.yaml` under `k8s.deployments`. Each item is either a component name (deployed alone) or a list of names (deployed in parallel within that step). When the project config has no `k8s` section, deploy falls back to the default rank `erun-backend-postgres → erun-backend-db → erun-backend-api → other`. The `erun` repo's own `.erun/config.yaml` declares `[erun-devops, erun-backend-postgres]` as a parallel first step (they share no state), then `erun-backend-db`, then `erun-backend-api`.
