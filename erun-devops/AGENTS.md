...
- The runtime image bakes agent skills under `/etc/erun/skills/<skill-name>/SKILL.md` from `erun-devops/docker/erun-devops/skills/<skill-name>/`. On every entrypoint run, `initialize_claude_config` and `initialize_codex_config` install each skill into `~/.claude/skills/<skill-name>/` and `~/.codex/skills/<skill-name>/` only when the destination file is absent, so user edits made inside a remote environment are never clobbered. Editing a baked SKILL.md is an image content change and follows the standard tag-bump rule above.
