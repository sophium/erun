...
- Bump the published tag whenever a base-image Dockerfile changes content. Cluster pods pull by tag with `imagePullPolicy: IfNotPresent`, so reusing the same tag with new content does not roll out — kubelets keep the old cached digest. The `erun-dind` image uses `<docker-version>-<rev>` (for example `28.1.1-1`); when the Dockerfile changes, increment the trailing `-rev`, update both chart references that default to `erun-dind:<tag>` (`erun-devops/k8s/erun-devops/templates/service.yaml` and `erun-common/assets/default-devops-chart/templates/service.yaml`), and clear the corresponding `docker.fingerprints` entry so the next release publishes a fresh fingerprint for the new tag.
