...
type serverRunner func(context.Context, eruncommon.BuildInfo, erunmcp.HTTPConfig, erunmcp.MetricsConfig, erunmcp.RuntimeConfig) error
...
func run(args []string, stderr io.Writer, info eruncommon.BuildInfo, runServer serverRunner) int {
	...
	metricsCfg := erunmcp.MetricsConfig{}
	...
	flags.StringVar(&metricsCfg.Host, "metrics-host", erunmcp.DefaultMetricsHost, "Host interface to bind the Prometheus metrics server to")
	flags.IntVar(&metricsCfg.Port, "metrics-port", erunmcp.DefaultMetricsPort, "Port to bind the Prometheus metrics server to")
	flags.BoolVar(&metricsCfg.Enabled, "metrics-enabled", true, "Serve the Prometheus /metrics endpoint")
	...
	if err := runServer(ctx, info, cfg, metricsCfg, runtime); err != nil {
