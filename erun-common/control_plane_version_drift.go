...
func ResolveControlPlaneVersionDrift(ctx Context, result ListResult, deps CloudDependencies, resolvePublished RuntimeRegistryVersionResolverFunc) ControlPlaneVersionDrift {
	...
	for _, provider := range planes {
		drift.Planes = append(drift.Planes, resolveOneControlPlaneVersionStatus(ctx, provider, deps.FetchPlatformInfo, publishedSemver, publishedOK))
	}
	return drift
}
...
func resolveOneControlPlaneVersionStatus(ctx Context, provider CloudProviderStatus, fetchPlatformInfo func(Context, string) (PlatformInfo, error), publishedSemver semver, publishedOK bool) ControlPlaneVersionStatus {
	...
	deployedSemver, deployedOK := parseRegistryStableVersion(status.Version)
	if !deployedOK {
		return status
	}
	switch {
