...
var mcpToolDescriptors = map[string]MCPToolDescriptor{
	...
	// A build-cost investigation tool (root AGENTS.md #2274): reads the
	// per-step CPU/throttling/I/O already recorded to ~/.erun/timing/. Its
	// primary caller today is whoever is diagnosing build slowness -- often
	// an agent, same as the exec_* family above -- and a desktop Builds view
	// over the same records is deliberately deferred follow-up work, not yet
	// built; revisit AgentFacing once that view exists.
