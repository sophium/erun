...
- **The gate's `erun build` cannot verify the desktop app (#1933).** `make check`'s `test` stage has no Wails/webkit toolchain, so `erun-ui/playwright` never runs inside a `GATE` build even though `erun-ui/AGENTS.md` makes that suite mandatory for every desktop change — a green `GATE` build proved nothing about `erun-ui/**` until this was caught: four desktop PRs (#1911, #1925, #1926, #1927) merged through the gate with their own new Playwright specs never executed anywhere, and a real run against `main` afterward found `27` failures, including a regression in #1901's own spec that the gate had reported green. **Verified empirically, not assumed:** the Wails/webkit CGO toolchain (`build-essential pkg-config libgtk-3-dev libwebkit2gtk-4.1-dev libsoup-3.0-dev`) installs cleanly on the exact `golang:1.26.0` base the test stage already uses, `go build -tags "desktop,production,webkit2_41"` succeeds there, the compiled binary runs `--headless` with zero display (no Xvfb — `runHeadless` never calls `wails.Run`), and Playwright's own `install-deps chromium` supports that same base — so there is no platform blocker to running the suite inside the gate for real; #1937 tracks that fix, gated on paying two real costs first (every build in the monorepo gets slower under the current single-layer `make check` shape, and the suite has 27 failures to fix before it can gate anything). Until #1937 lands, `erun review record-build --gate` (`erun-common/desktop_coverage_gate.go`) refuses a successful call whose commit changes `erun-ui/**` unless `--desktop-playwright-verified` attests the suite was actually run against it — a fail-closed stopgap, not a fix: it makes the gap loud instead of silent, but does not make the gate run the suite itself. This refusal fires on the review's own `GATE` build; the independent `gate_runs` record (#1931) a driving skill also reports must not be left `RUNNING` or marked `FAILED` when that happens — the underlying `erun build` genuinely passed, so `erun-merge-queue-drive` reports the `gate-run` `INCONCLUSIVE` instead, the same non-verdict status a timeout or environment fault gets. See `erun-docs/docs/collaboration/merge-queue.md` § "The gate" and § "The desktop coverage gap".
