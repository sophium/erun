...
- Prefer integration scenarios over unit tests in `erun-cli` and `erun-common`. The integration suite is the single source of truth for coverage on those modules: it exercises the compiled binary end-to-end and is the only signal the gate enforces. Unit tests in those packages do not contribute to the gate, so they are pure duplication when the same statements are already reachable through a `--dry-run` scenario or a stub-driven scenario.
- When adding or changing behavior in `erun-cli` or `erun-common`, add an integration scenario first. Reach for a unit test only when the behavior is genuinely unreachable from the binary subprocess (for example, a pure parser or a branch gated on a platform the harness does not run on); document why in the test file.
- When a unit test in `erun-cli` or `erun-common` overlaps with an existing integration scenario, delete the unit test rather than maintaining both. Keeping both forks the source of truth, hides drift, and inflates per-module coverage numbers without moving the gate.
