...
- CI enforces this gate: `.github/workflows/integration-test.yml` runs `make integration-test` on every PR targeting `main` and on pushes to `main`. The gate that binds locally binds in CI; the `integration-test` job should be a required status check in branch protection so a `[skip ci]` head commit blocks the merge instead of bypassing the gate.
