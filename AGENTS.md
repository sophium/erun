...
**That exit-124 distinction stops surviving the moment `make` sits between `agent-gate.sh` and whoever reads the result.** GNU Make collapses any nonzero recipe exit to its own generic exit 2 — verified directly (`make -f` with a recipe that `exit 124`s still reports make's own exit as `2`) — so a caller that reads only `make check`'s own exit status (an external orchestrator polling this environment asynchronously over its own MCP edge and wrapping the whole invocation as one outer job, rather than re-invoking `make check` itself or querying the named inner job id directly) cannot tell a timeout from a real failure: both read back as exit 2. This produced exactly the false-red case above (erun#2114). The default, foreground-safe behavior (a coding agent's own bounded shell tool call bails at the first timeout with exit 124, per the design above) must not change to fix that — it cannot safely block longer without risking the exact foreground-timeout trap `agent-gate.sh` exists to avoid, and `ERUN_JOB_ID` being set is not a safe signal to tell the two kinds of caller apart (it is set on a coding agent's own foreground call just as often as on an asynchronous orchestrator's). A caller that is *not* foreground-constrained instead opts in explicitly with `AGENT_GATE_AWAIT_VERDICT=1`, which makes `agent-gate.sh` keep re-awaiting the same job across repeated bounded `job await` calls until it reaches a real verdict, so `make check`'s own exit status is only ever a genuine pass or a genuine failure — never a swallowed timeout.
