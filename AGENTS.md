...
- CI enforces this gate: `.github/workflows/integration-test.yml` bootstraps `erun` from the PR's own source and runs `erun exec raw make integration-test` on every PR targeting `main` and on pushes to `main` — the gate goes through erun's documented automation surface, not a hand-rolled step, so every PR also proves the CLI builds and executes. The gate that binds locally binds in CI; the `integration-test` job should be a required status check in branch protection so a `[skip ci]` head commit blocks the merge instead of bypassing the gate.
