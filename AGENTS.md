...
- A failing test or lint finding you encounter is yours to investigate and fix, even when your change did not cause it. "It was already broken" / "not caused by me" is never a reason to skip, ignore, suppress, or bypass it — no `--no-verify`, no `//nolint`, no `t.Skip`, no `test.fixme`, no commenting-out, no threshold bump. A flaky test — one that passes only sometimes — is a failing test too: make it deterministic (wait on observable conditions, never wall-clock sleeps or retries-until-green), never skip it, mark it `fixme`, or wave it through as "pre-existing flakiness". The pre-commit hook lints the whole module, so touching any file in a module makes that module's pre-existing findings yours to resolve. Default to fixing it in the same PR; only when the fix is genuinely out of scope, open a tracking issue and link it before proceeding — but a green commit hook and a green `make integration-test` are the floor, not an aspiration. Surfacing a failure as "pre-existing" without resolving or tracking it is the defect this rule exists to prevent.
