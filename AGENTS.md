...
- This repository intentionally has no hosted CI (#521): GitHub-hosted runners are ephemeral, which defeats erun's daemon-centric build caching, and erun provides its own build system and merge queue instead. Both halves of that queue are state in `erun-backend-api`, not a dedicated Job: promoting a review to `MERGE` is the environment's own cue to build the prospective merge of the review's source onto its *current* target and gate it with a real `erun build` itself, pushing only on green and reporting the outcome — `MERGED` is accepted only once the platform verifies the reported commit against the real repository, so it means a merge actually happened and a build actually passed, not a caller's assertion. An accepted (now actually-merged) review then enqueues a release, which the environment that earns it runs the same way. The client-side plumbing this needs now exists (`erun exec gate-merge`, `erun exec report-commit-status`, `erun review record-build --gate`, `erun review report-merged`, and the `erun-merge-queue-drive` skill chaining them — see `erun-docs/docs/collaboration/merge-queue.md` § "The gate"), but nothing runs it against **this** repository's own PRs today: there is no project registry at all — the repository a gate or review call acts on is a per-call argument (e.g. `erun review report-merged --remote-url`) — and `erun/ux` plus every code lane already holds a warm checkout of this repo with push credentials, so neither the environment nor the credentials are what's missing. What actually stops it running today is that no branch protection rule on `sophium/erun` requires the gate's status check — until that changes, this repository's own PRs still merge exactly as this bullet has always said: run the gate locally before merging (`make check`), and every erun-driven build anywhere else re-runs it via the image's test stage (see `erun-devops/AGENTS.md` § "Build Workflow"). Do not reintroduce GitHub Actions workflows for build or test gating.
