...
**A gate's verdict has three states, not two: pass, pass-with-orphan, and fail — never collapse the middle one into the third.** `EnvironmentJob.Succeeded` (`erun-common/job.go`) is deliberately strict — it is false for state `abandoned` (unsupervised background work left running in the job's own process group) even when the gated command itself exited 0, because a zero exit from a process that left work behind is not the same claim as a zero exit from a job that finished cleanly. That strictness is correct for the job record, but a caller reading only `job await`'s own exit status cannot tell that middle case apart from a genuinely nonzero gate exit — both come back nonzero. `agent-gate.sh` re-reads the job's own record after `job await` returns nonzero specifically to split them back apart: a real gate failure (state `exited` with a nonzero code, `gate-incomplete`, `unknown`, or anything else short of success) still fails the run; state `abandoned` with exit code 0 — a real leftover, not the false-positive `run.sh` self-counting bug root-caused in #1801 — passes, but only after printing a named, impossible-to-miss warning identifying what was left running. Never read "exited 0" alone as sufficient for a pass, and never fold a real orphan silently into a red: the orphan means work the gate started never reached its own verdict, which is exactly the kind of unreported outcome § "One Agent Job Is One Run" above already treats as a defect.
