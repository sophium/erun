...
- CI enforces this gate as the delivery pipeline's own build step: `.github/workflows/build.yml` bootstraps `erun` from the PR's source and runs `erun build` on every PR targeting `main` and on pushes to `main`. Tests run inside the build — the `erun-devops` image's test stage runs `make integration-test` and the builder stage depends on its marker — so a failing gate fails the build and no image is tagged (see `erun-devops/AGENTS.md` § Build Workflow). The gate that binds locally binds in CI; the `build` job should be a required status check in branch protection so a `[skip ci]` head commit blocks the merge instead of bypassing the gate.
