...
- **A release's 20-30 minute wall-clock is almost entirely one real, mostly-irreducible cost, not idle waiting (#1985).** Measured from this repository's own recorded `~/.erun/timing/release-*.json` runs: `publish` (the per-image build+push phase) is 85-93% of total duration in every non-degenerate run; `sync-remote`, `verify-publication`, `push-release-tag`, `sync-packaging-checksums`, `post-release-version-bump`, and the final `push` are each under 10 seconds. Within `publish`, 9 of this repository's 11 images cache-hit in seconds because their content didn't change; the other two dominate every release because they bake `ERUN_VERSION` into their own artifact (see the `ERUN_VERSION`-fingerprint bullet under `erun-devops/AGENTS.md`'s Build Workflow section) and therefore *must* rebuild on every release regardless of code churn: `erun-backend-api` (~5-6 min, a Go compile) and `erun-devops` (~10-15 min typically, the desktop CGO/webkit compile plus the test stage's from-source golangci-lint build — occasionally 30+ min under a cold BuildKit cache). One observed run with a fully warm cache and no version change completed in 4m26s, proving the floor is low when nothing has to rebuild — the 20-30 minute norm is the version-fingerprint-forced rebuild of those two images, not overhead. They build one at a time by deliberate, already-documented design (`runDockerBuildsSequentially`, `erun-common/build_run.go`): the push/release path pins concurrency to 1 because those builds share one in-pod docker daemon, and `erun-devops/AGENTS.md`'s dind memory-limit findings show real, already-encountered memory-exhaustion risk when independent build steps run concurrently against that shared daemon. `erun-backend-api` and `erun-devops` have no dependency on each other, so running them concurrently is possible and would save roughly the smaller one's duration (~5-6 min, ~20-25% of total) — but only safely once a memory-aware concurrency limiter (the same shape as `scripts/parallel-gate.sh`'s `width` mode) exists for the shared dind sidecar; doing it without one risks the identical resource-exhaustion class #1805 already fixed for one image's internal stage race, at the scale of two whole images instead of two stages. Treat this as a bounded, non-urgent future optimization, not a release blocker: even a perfect fix leaves `erun-devops`'s own ~10-15 minute compile as the floor.
- **Merge-triggered releasing is already half-built, and the missing half is bigger than "wire up the queue" (#1985).** `erun-backend-api/AGENTS.md`'s "Release Queue" section already records that every accepted merge auto-enqueues a `QUEUED` row via `ReleaseTrigger.TriggerRelease`, and that nothing claims or runs one (`ClaimNext`/`RecordOutcome`/`ExpireStale` exist and are tested, but no route, CLI/MCP command, or skill calls them — contrast `erun-merge-queue-drive`, which fills that exact role for the gate side of the same queue). Two things follow that section doesn't state: first, building the missing executor (a claim+record-outcome route pair, plus an `erun-release-queue-drive`-style skill/command) runs into the identical per-agent-pod platform-alias wall #1969 already documents for the merge-queue driver — an automated release executor cannot read or claim the queue from an agent pod today any more than a gate-run reporter can. Second, `TriggerRelease` only fires from `acceptMerged`, so it only covers merges that land through erun's own review queue; a plain GitHub "Squash and merge" (a real, currently open path — `sophium/erun`'s `pull_request` rule requires no approving review and no status check, per `erun-backend-api/AGENTS.md`'s "GitHub branch protection cannot tell the queue's push from a bypass") never enqueues a release at all. Full on-every-merge coverage needs that path closed too, or the cadence policy below has to explicitly accept partial coverage.
- **Cadence policy, pending the queue-execution gap above closing: release when merges accumulate, not when someone notices a lag.** Automatic on-merge releasing is designed for at the data-model level but has no executor and is blocked on #1969 the same way the merge-queue driver was, so it is not a near-term fix. Until an executor exists, state and follow an explicit trigger instead of ad hoc judgement: release after every 3-5 merges to `main`, or immediately after any merge an operator has flagged as unblocking other lanes (the exact situation that cost multiple lanes real time on 2026-09-03 when #1884/#1892/#1898 sat merged-but-unreleased), whichever comes first. A stated numeric or event-based trigger is strictly better than "an operator noticed" because it does not depend on anyone noticing.
