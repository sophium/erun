...
func newReleaseCmd(findProjectRoot common.ProjectFinderFunc, runGit common.GitCommandRunnerFunc) *cobra.Command {
	...
	cmd.Long = "Cut a release from the current branch.\n\n" +
		"Resolves the next version, updates the version files, commits, tags, and pushes the tag to origin. It never builds or publishes an image or a chart — that is `erun build --release`'s job (and `erun push`'s, for republishing). Which branch releases as a stable version vs a prerelease comes from .erun/config.yaml.\n\n" +
