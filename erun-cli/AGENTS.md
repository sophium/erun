...
- **The same rule applies one level down, to flags.** Adding a flag to a command that already has a desktop surface used to be invisible to that gate, which is how review discovery's seven CLI filters shipped with no desktop counterpart (#2141). Every own (non-inherited, non-`Hidden`) flag on an operator-facing command is now audited on its own, so a new flag needs either a real reference in `erun-ui/frontend/src` / `erun-console/src` or an entry in one of two maps in `erun-cli/cmd/command_tree.go`: `cliOnlyAgentFacingFlags` when the flag is structurally about the CLI's own invocation (a prompt bypass, shell integration, an exit code, a scripted-composition switch, an in-pod-only repair) and therefore could never have an affordance, or `knownUnsurfacedFlags` — the shrink-only baseline — only for a gap that predates the gate seeing flags. A fresh failure never belongs in the baseline; give the flag an affordance or say in `cliOnlyAgentFacingFlags` why none could exist.
