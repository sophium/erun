...
- **Exception: `dialog.tsx`'s `grid-cols-1` and `min-w-0` classes are a deliberate, permanent departure from stock shadcn output**, not drift. Stock `DialogContent` renders `display: grid` with no explicit `grid-template-columns`, so the browser sizes the implicit single column to the max-content width of its widest descendant — an unbroken string (a long file path, a UUID) with no `min-w-0` above it forces that column, and every sibling grid item with it, wider than the card's own clamped box, spilling in-flow content past a card whose background/border still paint at the correct width. `grid-cols-1` (Tailwind for `grid-template-columns: repeat(1, minmax(0, 1fr))`) clamps the column to the container; `min-w-0` on `DialogHeader`/`DialogFooter` keeps them shrinkable inside it. `yarn shadcn:check` will therefore always show a diff on `dialog.tsx` — that is expected, not a regression to chase. If `shadcn add dialog --overwrite` is ever run, reapply these two class changes afterward.
