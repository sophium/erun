...
Build artifacts land alongside it. Anything the Agent writes to the pod's outputs directory — a cross-compiled `.exe`, a report, a bundle — is mirrored into a **read-only `.erun-outputs` folder** next to the synced source. That's how a Windows binary built in the Linux pod reaches your machine: it syncs down, and you run or debug it from the desktop (the same artifacts are also available via **Download** on the env's Outputs dialog). On a Mac, an arriving macOS binary is ad-hoc signed and made runnable on the way in — macOS kills an unsigned one on launch without saying so — and the desktop posts a notification saying what it did, or why it could not. See [`erun outputs`](/cli/outputs#macos-binaries-arrive-runnable) for the exact rule.
