...
Any error banner in the titlebar carries the same reporting action, ahead of the browser form: a known remedy (Deploy, Wait longer) leads, and **Report a bug** always follows it — or leads on its own when there's no remedy to name. Rather than a form you fill in, it hands the failure to an Agent: the Agent searches for a matching open issue first and proposes commenting on it instead of filing a duplicate, then drafts the report from the full captured output (not trimmed to fit a link) in its own session for you to review before anything is submitted. If no Agent is available to draft it, the titlebar falls back to the same prefilled-browser-form path as the console's own button, and says why.
