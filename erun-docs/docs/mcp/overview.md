...
| Tool | Read/Work | Purpose |
|---|---|---|
| `review_list` | Read | List reviews, narrowed by any combination of `repository`, `targetBranch`, `sourceBranch`, `status`, `authorUserId`, and `reviewerUserId`. `repository` is not defaulted from the checkout — a listing is how the agent finds work across every repository the tenant serves. `mine`/`waitingOnMe` resolve to the caller's own user id via `platform_whoami`. |
| `review_show` | Read | Fetch one review together with its comment threads and recorded builds. |
| `review_create` | Work | Open a review. `name` is the eventual squash-merge message and is unique per repository among reviews that can still land — a `CLOSED` review's name is free to reuse. `repository` names the repository the branches belong to, defaulting to the pod's own checkout origin; it is what keeps two repositories a tenant serves from sharing a merge queue, so a review opened without one is invisible to every repository's queue. `sourceBranch` must already be pushed (`exec_push`) — the platform can only fetch what has actually landed on the remote. |
...
| `review_queue_list` | Read | List one repository's merge queue for a target branch, in queue order. `repository` names the queue, defaulting to the pod's own checkout origin: a tenant may serve more than one repository and they share branch names. |
| `review_queue_advance` | Work | Advance a repository's merge queue head to `MERGE`, starting that review's merge-gate build — a real, immediate mutation of shared control-plane state. Fails if the queue is empty or its head is not `READY`; refuses with the unresolved comment thread count when the head still has open threads (resolve them with `review_resolve`, or use `review_queue_override-advance`); refuses `MERGE_QUEUE_AMBIGUOUS` when no `repository` was given and the branch's queue holds more than one repository's reviews; and refuses, naming the occupying review and its source branch, while another review already holds that queue's single `MERGE` slot — wait for it, or free it with `review_requeue`. |
| `review_queue_override-advance` | Work | Bypass `review_queue_advance`'s unresolved-thread gate and advance anyway. `repository` names the queue exactly as above. `reason` is required and is recorded in the platform's audit trail alongside the caller's identity — a deliberate, accountable escape hatch, not a routine way to advance the queue. |
...
| Tool | Read/Work | Purpose |
|---|---|---|
| `jobs_list` | Read | List the tenant's queue, the live work first, narrowed by any combination of `status`, `environmentId`, `issueRef`, `scope`, and `actorId`. Each entry names what is being done, by whom, and how long it has been going. `RUNNING` is work in flight, and so is `PLANNED` — work recorded before it starts, which the sweep deliberately leaves alone; `ABANDONED` means its actor stopped updating it and the platform swept it — read it as dropped, not as failed. An empty queue is `[]`, never `null`. |
| `jobs_show` | Read | Fetch one job by `jobId`, including the scope it claims and the in-pod job id it mirrors, when there is one. |
| `jobs_start` | Work | Record that this actor is starting a piece of work: `jobType` (one of `fix`, `review`, `gate`, `release`, `deploy`, `investigate`, `plan`, `triage`, `maintenance`), `summary`, `actorId`, and optionally `actorKind`, `environment`, `issueRef`, `scope`, `status`, and `localJobId`. `status: PLANNED` records work that has not started — a `triage`/`plan` item parked against its issue, visible in the pipeline view before any code exists, and deliberately never swept to `ABANDONED`. With `scope` set this is a **claim**: if an open job already holds that scope, the call is refused with `409` naming the holder — its `actorId`, its prose `summary`, and when it `started` — so you can pick up something else instead of duplicating the work. A `PLANNED` job holds its scope too. Returns the new job's id; pass it to `jobs_finish`. The summary is prose describing the work, never the command that performs it — a summary that is only a shell command is refused. |
| `jobs_finish` | Work | Move `jobId` forward: `RUNNING` opens a `PLANNED` job — the one move that begins work — while `SUCCEEDED`, `FAILED`, `ABANDONED`, and `SUPERSEDED` close it; also refreshes its `summary` or records the `localJobId` it mirrors. A job that has already finished cannot be updated (409): its outcome is the record coordination and reporting both read. |
...
It rides along as a `sizing` field — the same verdicts and evidence window `resize` reasons from — so a caller checking on an environment does not need a separate `resize` call just to see it. The verdict is derived from this reading together with the environment's retained usage history, which its own pod monitor keeps and which is why the block appears here in full: [`erun usage`](/cli/usage) and [`erun doctor`](/cli/doctor) print a sizing block too, but a host-side reading has only the live counters behind it — the raise direction (a peak at the limit, or a recorded OOM kill) still fires there, while the shrink direction, which needs a day of quiet evidence, reads as insufficient-evidence — and a host-run [`erun list`](/cli/list) reports none for it. Omitted when nothing has been observed yet.
...
Pass `controlPlanes: true` to additionally check every configured erun-hosted control plane's deployed version, and the deployed version of each surface that plane links to (its console and its docs site), against the newest version erun's own registry has actually published — deployed-vs-published, not deployed-vs-main; the result gains a `controlPlaneVersionDrift` field alongside the ordinary list result. This makes real network calls (each plane's own `GET /v1/platform`, each linked surface's `GET /version.json`, plus a registry lookup); pass `preview: true` to trace which planes, linked surfaces, and registry lookup would be checked instead of making any call. See [CLI flag spec · Control plane versions](/agent-reference/cli-flags#control-plane-versions) for the full field contract.
