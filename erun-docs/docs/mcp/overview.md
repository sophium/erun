...
| `activity_lease_release` | `erun activity lease release` | `released` (true only if a matching lease was actually removed) and, on a no-match, `note` naming the mismatch (e.g. the id is held exclusively under a different scope) — plus every lease still held on the env. Never errors on a no-match, so a cleanup call is always safe to repeat; check `released`/`note` to tell "released it" from "nothing matched". |
