...
The `file://` anchor is **sticky across redeploys**: the deploy that injects the key records its path on the env ([`mcpauthpublickeypath`](/reference/configuration#envconfig)) and every later deploy of the runtime chart rethreads it, so a plain version bump cannot leave the edge unauthenticated. Turning the anchor off takes the explicit `erun deploy --no-mcp-auth`, and a deploy that would drop authentication the live release still has is refused — see [`erun deploy` · MCP-auth stickiness](/agent-reference/cli-flags#deploy-mcp-auth).
