...
| Inputs | One or more supplied review IDs already at MERGE, a common target, verified remote source SHAs, platform/git/build access, and the owning environment. Platform access is a hard precondition of the machine running the drive, not something the drive can arrange: an agent environment has no erun platform alias and cannot obtain one, so the drive stops before claiming the environment instead of reserving it for work it could never record. Non-MERGE reviews are dropped; no survivors, mixed targets, or missing refs stop the drive. The normal API permits only one MERGE review per tenant/target: multi-source git composition does not establish general multi-review acceptance. |
