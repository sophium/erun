...
| Outputs | In-pod development over erun MCP, and read-only host review of a pod-backed environment's worktree; a host environment has no pod, so it is worked in its own directory directly. Host-native verification of cross-built outputs. Remote mirrors deliver artifacts under `.erun-outputs/`; local-agent artifacts need download. Native GUI compilation is the host-build exception, using a separate owned copy outside all review directories — a host environment is already exactly such a copy. Restarts use `erun app restart` and an ID-specific `RESUME-NOTE.<id>.md` naming task, delivered work, in-flight job IDs, and first checks. Final reports include evidence, gaps, and assumptions. |
