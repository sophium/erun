...
See [activity leases](/agent-reference/idle-policy#activity-leases) and
[exclusive claims](/agent-reference/idle-policy#exclusive-claims) for lifecycle
semantics, [desktop orchestrators](/desktop/orchestrators) for configuration, and
[restart recovery](/cli/app) for the restart operation. Release orchestration
verifies publication before public tag push; release does not itself deploy.
...
See [merge queue](/collaboration/merge-queue) for acceptance and
[bypass reconciliation](/collaboration/merge-queue#reconciling-a-bypass) for
ruleset evidence. The [desktop gate](/collaboration/merge-queue#desktop-coverage-gap)
runs real headless tests, not a manual attestation; native-OS gaps remain distinct.

#### Infrastructure-failure classification

1. Gate-run start/report examines only caller-reported `FAILED` status (trimmed,
   case-insensitive). Other requested statuses are unchanged.
2. It combines `failingStep`, the `logRef` value itself, and the contents of
   `logRef` when it names a readable local regular file no larger than 1 MiB.
   Missing/unreadable files, larger files, job IDs, and URLs contribute no file
   content; the classifier does not fetch remote logs.
3. It matches this text case-insensitively against these signatures, any one of
   which is sufficient: `failed to resolve source metadata`,
   `failed to fetch oauth token`, and `tls handshake timeout`.
4. A match changes the gate-run report to `INCONCLUSIVE` and emits a trace in
   both preview and real execution. Unmatched failures remain `FAILED`.
5. A failed review `GATE` build applies the same signature matcher to its
   `failureDetail`, but refuses a match rather than writing a false failed build:
   that record has only a boolean success field. Report the gate-run inconclusive
   and preserve the review for a separately authorized recovery attempt.

This classifier recognizes only the listed signatures. An unknown outcome or
another established infrastructure fault still needs an explicit inconclusive
report; a wrapper timeout alone is not evidence that the underlying job ended.
