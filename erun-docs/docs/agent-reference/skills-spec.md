...
| Outputs | The target and source fetched and squash-merged onto a fresh local checkout of the target (`erun exec gate-merge`); the result gated with `erun build` (never `--release`, since the gate publishes nothing); the outcome recorded as a `GATE`-kind build (`erun review record-build --gate`) — successful or failed, this is what actually moves the review to `FAILED` on a bad gate; on success only, the target branch pushed (`erun exec push`) and the review reported `MERGED` (`erun review report-merged`), which the platform verifies against the pushed commit and the target branch's real tip rather than trusting. Alongside the review's own `GATE` build, a `gate_runs` record (erun#1931) is started (`erun exec gate-run start`) as soon as the prospective merge commit exists and reported `PASSED`/`FAILED`/`INCONCLUSIVE` (`erun exec gate-run report`) at each terminal point, so `erun gate list` shows this attempt as currently gating and, later, what it decided — independent of the review-specific reporting beside it. When the squash commit touches `erun-ui/**`, the skill names the desktop-coverage gap before recording the build (`erun build`'s own test stage cannot run `erun-ui/playwright` — see [Merge queue § The desktop coverage gap](/collaboration/merge-queue#desktop-coverage-gap) and issue #1933) and only passes `--desktop-playwright-verified` once that suite has actually been run and watched pass; `record-build --gate` refuses a successful call that changes `erun-ui/**` without that attestation, so this is enforcement, not just narration. Once `MERGED` is reported, the source branch's open pull request (if any) is closed on GitHub with the landing commit recorded in a comment (`erun exec close-pr`) — a no-op when there is no open pull request. Reports the review id, source and target branches, the gate build's id and commit, the resulting status, and whether a pull request was found and closed, then stops. |
| Error behaviour | `erun` not on PATH → stops before touching git. The named review is not `MERGE` → stops, naming `erun review queue advance` as the missing prerequisite. The source branch is gone from origin → stops before any mutation. A squash conflict → reports a `gate-run` `FAILED` with no trackable running phase (`failing-step: git merge --squash`) and stops. A failed `erun build` → records a failed `GATE` build and a `FAILED` `gate-run` (`failing-step: erun build`) against the best available commit and stops; never fabricates a commit to report against, and never resolves a conflict itself. The build's own outcome is genuinely unknown (an environment fault, a wrapper timeout) → the `gate-run` must report `INCONCLUSIVE`, never `FAILED` — a non-verdict is not a red one. `record-build --gate` refusing because desktop coverage is unattested (or for any other reason, after the `erun build` itself already passed) → the same `INCONCLUSIVE` report, never `FAILED`: the build was green, but the review has no recorded `GATE` build to show for it, so the gate reached no real verdict on mergeability. A push refused after a successful gate build (commonly a non-fast-forward) → stops naming it as an anomaly — the queue's own one-merge-in-flight-per-branch invariant means nothing else should have moved the target — rather than retrying blindly; the `gate-run` already reported `PASSED` stands, since the build itself was green. A `report-merged` refusal (`409 MERGE_NOT_VERIFIED`) → surfaces the platform's own reason rather than retrying. A pull request whose head moved since the gate fetched it → the close step refuses, loudly, rather than closing over content the gate never saw; the review's `MERGED` status stands, and the pull request is left open for a human to reconcile. |
