...
For the supplied `--version`, `push` always builds each image from its source context (never a prebuilt bare tag), pushes per-arch tags, assembles the manifest list, then for every component that ships a co-located chart (`erun-devops`, `erun-powerdns`, `erun-backend-postgres`, `erun-backend-db`, `erun-backend-api`, `erun-docs`) runs `helm package` + `helm push` to `oci://<registry>/charts` at `--version` and verifies with a `helm pull` round-trip. Chart publishing is decoupled from the image push: a **version-pinned base** (`erun-powerdns`, `erun-backend-postgres`) keeps its image at the upstream pin and is not re-pushed at `--version`, but its chart still publishes at `--version` so platform deploys resolve it. Charts publish under `/charts`, separate from the same-named image repo so a chart never collides with its image at the same ref. There is no environment-type branch. [`erun release`](#erun-release) reuses this step for all its publishing.
