...
When no coordinate answers — an unreadable or unauthenticated registry, or a version published nowhere — the search falls back to rung 2, which is what resolution produced before rung 3 existed, and traces `deploy: no runtime chart candidate published <v>; falling back to erun-devops in R`. The subsequent chart pull then fails as `MISSING_CHART_IN_REGISTRY`, whose message enumerates every coordinate probed and the three ways out: [`erun init --runtime-registry <host>`](#erun-init) to record where ERun's artifacts live, `erun push --version <v>` from the project that owns a `<tenant>-devops` umbrella, or [`--runtime-chart`](#deploy-runtime-chart) / `EnvConfig.runtimechart` to name the chart outright.
