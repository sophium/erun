...
Each environment's version comes from the same `ResolveErunVersion` config-only resolution the full listing's `runtime-version:` line uses (see [Release lines](/cli/list#release-lines)) first. When that resolves to nothing — e.g. a deploy that never recorded a resolved runtime image, or, the case that motivated the fallback below, a tenant whose runtime image rides its own release line rather than erun's stock `erun-devops` (erun#2093, `ResolveRuntimeVersionLine`/`ResolveErunVersion` in `erun-common/runtime_version_line.go` can never parse an erun version out of a tag scheme it doesn't own) — `ResolveTenantVersionDrift` (`erun-common/version_drift.go`) falls back to a live probe of that environment's own MCP edge: it performs the `initialize` handshake (`ProbeMCPServerVersion`, `erun-common/mcp_client.go`) against `http://127.0.0.1:<mcp-port>/mcp` and reads the erun version straight out of the reply's `serverInfo.version` — provenance-independent, since it names the version of the binary actually running there regardless of which image or config carried it. This is a fallback, not a replacement: an environment whose config already resolves a version is never probed.

The fallback probe only fires when the environment's local MCP port actually answers (`CanReachLocalMCPEndpoint`); an environment nobody has opened, or one that's genuinely stopped, is left with `version=none` exactly as before, with no probe attempted whose only conclusion could be "unreachable" anyway. Three outcomes follow from there, captured in `EnvironmentVersionStatus`:

- **Live-probed successfully** — `version` reports the probed value (JSON: `version`), and `behindMax` is computed against it exactly as it would be for a config-resolved version.
- **Reachable, but no version could be determined** (the handshake errored after the port answered, or `serverInfo.version` came back empty) — `versionUnresolved: true` (JSON: `versionUnresolved`), rendered as `version=none [up but version unresolved]` in text. This is treated as a finding by `--fail-on-drift` (see [Exit codes](#erun-list) below): an environment that is up and answering but still unreadable is itself drift-shaped, unlike one that was never opened.
- **Unreachable** — `version` stays empty and `versionUnresolved` stays `false`/omitted. Not a finding.

Under `--dry-run`, no probe call is made at all (matching `--control-planes`' own no-network-call dry-run contract below): every environment that would be probed is traced instead (`list: <tenant>/<env> has no erun version recorded in config; would probe its MCP edge at <endpoint> for the running version`), and its version stays unresolved for the run.
...
`max version` is the newest version observed among the tenant's *own* environments — after any live-probe fallback has resolved — not the newest erun has ever published, which is `erun version`'s/`erun upgrade`'s registry-latest concern. `[behind max]` marks an environment whose version parses lower than the max; an unparseable or missing version is shown bare (never guessed at) and excluded from the max computation. The `gate:` block only prints when `--gate-environment` is given; its own version is read from the same already-resolved statuses (including any live-probed fallback), never probed a second time. `behind:` has three readings: `no` (the gate carries the max, or ties it), `yes -- outdated relative to <envs>` (naming every environment running a newer version), and `unknown (gate's own erun version could not be resolved from config)` when the gate's own version can't be read — reported explicitly rather than folded into a silent `no`, since a gate older than the code it gates can pass a change that would fail on current code.
...
| `1` | `--gate-environment` without `--tenant`; unknown `--tenant`; unknown `--gate-environment`; `--control-planes` combined with `--tenant`/`--gate-environment`; `--fail-on-drift` without `--tenant`/`--control-planes`; or `--fail-on-drift` was passed and the report found drift (an environment behind max, an environment that is up but its version couldn't be determined even via the live MCP-edge probe (`versionUnresolved`), a behind/unresolved gate, an unreachable/behind/ahead plane or console, or an unresolved published baseline) — never while `--dry-run` is also set, since nothing was actually probed. An environment nobody has opened, or one that's genuinely stopped, is not itself a finding. |
