...
| `--gated-commit <revision>` | string | Empty for an ordinary build. | — | Names the revision this build must be a build of, folded into every image's fingerprint. The merge queue's gate build passes the prospective merge commit it gates, so the gate can never promote an image built from a different commit — including the branch tip whose tree is identical after a rebase onto the same target tip. Ordinary builds keep their content-only identity and still promote across commits that leave the build context untouched. |
...
3. **Mint the version.** Default: append the snapshot suffix `-snapshot-<UTC-timestamp>` to the resolved base. With `--release` / `--version` / a build-dir version: use the bare version. Compute the per-image fingerprint: content only, plus the consumed `ERUN_VERSION`, the component chart, and — for a build that names one — the `--gated-commit` revision, which is what stops a gate build from promoting an image built from a different commit.
