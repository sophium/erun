...
6. If `--deploy`: compose push + deploy at the minted version (operator-convenience shortcut). If `--release`: stamp and tag the version the same way `erun release` does, then build and reuse `push` to publish the release-tagged variants and chart, and verify they resolve. Programmatic callers skip both and orchestrate the primitives themselves.
7. Exit `0`.
...
`erun release` is version paperwork only: it resolves the version, stamps it into the chart and package-manager metadata, commits and tags locally, pushes the tag, syncs packaging checksums, prepares the next patch version, and pushes the branches. It never builds, publishes, or verifies an artifact. Building and publishing that version's images and the runtime chart is [`erun build --release`](#erun-build)'s job (it stamps and tags the same way internally, then builds and reuses [`erun push`](#erun-push) to publish, then verifies); `erun push --version <version>` republishes on its own. See [Release version policy](/agent-reference/release-policy) for the version-pattern rules and the publishing contract; the `erun release` flag set is `--dry-run`, `--force`, and `--output`, and is documented on the [Operator page](/cli/release).
