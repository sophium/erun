...
| `--dind-cpu <value>` | Kubernetes quantity | A **new** env records nothing here, and each deploy derives the limit from the machine it runs on (the node's cores less two, never below one); an **existing** env with a recorded `EnvConfig.runtimedindpod.cpu` keeps it. | Must match the Kubernetes `Quantity` grammar (`m`, plain integer, decimal). | `EnvConfig.runtimedindpod.cpu` — the `erun-dind` sidecar's own limit, independent of `--runtime-cpu`. Supplied alone it merges — naming only the CPU leaves the recorded memory where it was. Name it to pin the limit instead of deriving one, which is what a host driving a *remote* cluster should do: the derivation measures the machine the deploy runs on. |
