...
| `--runtime-memory <value>` | Kubernetes quantity | A **new** env takes `16384Mi`; an **existing** env keeps `EnvConfig.runtimepod.memory`. | Must match the Kubernetes `Quantity` grammar (`Ki`, `Mi`, `Gi`, …). | `EnvConfig.runtimepod.memory`. Merges with `--runtime-cpu` the same way. Sized for an in-pod `make check-gate` (the same thirteen-target gate the `erun-dind` sidecar runs during an image build), so an env that only serves an app can be trimmed below it. |
