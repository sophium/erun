...
| `--gate` | bool | `false` | — | Declares this the merge queue's gate build. Two things follow. A Dockerfile whose `test` stage a later stage copies from (`COPY --from=test`) is the project's own gate, and this build executes that stage with `--no-cache-filter <stage>` rather than letting BuildKit serve it from its layer cache — a replay runs no `make check` and exits 0 exactly like a real gate, which is why it cannot be accepted as one. Independently of this flag, any build whose gate test stage BuildKit wholly replayed is **refused** (naming the image and `--gate` as the way to execute that stage): the exit status is all [`erun review record-build --gate`](/cli/review#review-record-build) reads. Rejected with `--release`, `--deploy`, or `--e2e` — a gate is a verdict on a tree and publishes and deploys nothing. |
...
| Built-in default | `15m0s`. Sized for a cold pull of the runtime image, not for a serving app: every runtime chart renders `strategy: Recreate`, so the previous pod is torn down before the replacement is scheduled and the image is pulled with the environment running nothing. Measured: `ghcr.io/sophium/erun-devops` is 4.78 GB on disk / 1.14 GB compressed, and a cold uncapped pull took 386s on a fast link — longer than the `5m0s` this replaced, before any container started. |
...
- **Keep waiting (image still pulling).** A container in `ImagePullBackOff` or `ErrImagePull` whose message is **not** a permanent rejection is treated as a pull in progress — a large image on a slow or rate-limited registry legitimately cycles `Pulling → ErrImagePull → ImagePullBackOff → retry`. The watcher does not abort; it keeps waiting up to `<t>` and prints `pod <p>: <c> Pulling image (<reason>)` status lines so the wait is visible. `helm --timeout` is the only bound on this case, so when it expires mid-pull the deploy reports that the wait and not the rollout ended — naming the containers still pulling and the fact that the environment is running no pod until the image lands — instead of only helm's `Progress deadline exceeded`.
- **Abort early (real failure).** When a container reaches a state that will not recover, the watcher sends `SIGINT` (then `SIGKILL` after 15s) to the helm process and the deploy fails immediately with `deploy failed early: pod <p> container <c> <reason>: <message>` rather than waiting out `<t>`. The grace before the kill is what lets helm release the lock it holds from the moment it records the upgrade as pending: a hard kill leaves the release in `pending-upgrade`, and every later deploy of that environment fails against it until the metadata is cleared. The terminal states are:
...
| `--dry-run` | bool | `false` | Only meaningful alongside `--control-planes`. Traces which control planes, which of their linked surfaces, and which registry lookup would be checked, without making any network call, and prints `Dry run: control plane version check planned; see trace for the planes and registry lookup that would be probed.` |
| `--fail-on-drift` | bool | `false` | Makes this one invocation exit non-zero when any plane or a surface it links to is behind/ahead of published or unreachable, or the published baseline itself couldn't be resolved — see [Exit codes](#erun-list) below. Never fires under `--dry-run` (nothing was probed). |
...
**Each reachable plane's own `GET /v1/platform` response also names the surfaces it links to** — its console's URL (`consoleUrl`) and its documentation site's (`docsUrl`). Neither is configured as a separate alias: a plane advertises where they are, and the command resolves each. When one of those fields is non-empty, the command additionally calls that surface's own unauthenticated `GET /version.json` (a static file stamped from `ERUN_VERSION` at build time — `erun-devops/docker/erun-console`'s image for the console, the published docs site for itself) and reports the result nested under the plane as a `console` or `docs` field (`VersionSurfaceStatus`: `url`, `reachable`, `unreachableReason`, `version`, `reason`, `behind`, `ahead` — the same shape and the same published baseline for both, and the same as the plane's own fields).

The two are separate fields rather than one folded surface because they are published by different deploys: a docs site is uploaded on its own, so a current console says nothing about whether the docs served match the release the plane runs. A plane whose response carries no `consoleUrl`, or no `docsUrl`, gets no `console`/`docs` field for it at all (omitted from JSON, no `console:`/`docs site:` line in text) — an absent surface reads as absent, never as a guessed or current one. The plane's own reachability and each linked surface's are independent: a reachable plane can have an unreachable console, an unreachable docs site, both, or neither.

Each surface check distinguishes a transport failure from a content failure, because they point at different remedies. A request that never gets an HTTP response (connection refused, DNS failure, TLS failure, timeout) is `reachable: false` with `unreachableReason` set (prefixed `fetch <surface> version: `, naming which of the plane's surfaces did not answer), same as the plane's own check. A request that *does* get an HTTP response, but the body isn't the expected `{"version": "..."}` JSON document — most commonly an SPA's `index.html` served by a wildcard fallback route for a build whose nginx config doesn't yet exact-match `/version.json` — is reported `reachable: true`, `version: "unknown"`, with `reason` naming the HTTP status code and `Content-Type` it actually served (e.g. `/version.json returned 200 text/html (expected application/json)`). The surface answered, so it is never folded into `reachable: false`; it also never gets a `behind`/`ahead` verdict, since there is no parsed version to compare. `reason` is only ever set alongside `reachable: true` and an unparseable `version`; it is distinct from `unreachableReason`, which is only ever set alongside `reachable: false`.
...
The resumption restates the task the run was originally given, recovered from the argv the job's own record kept (`command`), ahead of the outcome it is being resumed for. That is deliberate: a one-shot run's final message is the only channel its result has, and a resumed turn told *only* about a failure answers only the failure — so a lane whose reproduction is a probe that fails on purpose would spend its resumed turn accounting for the red it meant to produce, and the finding would be lost while the job still read as a clean exit. With the task restated, the resumed turn's final message must still deliver the answer to the question it was originally asked. When the argv is not recoverable — a record written by a differently-shaped invocation, or a job that ran no agent argv — the prompt is unchanged and nothing is invented in its place.
