...
| Released by | `erun activity lease release --id <id>`, or the MCP `activity_lease_release` tool. Releasing an unknown or expired lease always exits 0/never errors — a cleanup trap must never fail a job that already finished — but it is reported honestly: the CLI prints `no lease released: ...` (never the success wording) and names the fix when the id is actually held under a different shape (a plain lease vs an `--exclusive` claim, or a different `--scope`); the MCP tool returns `released: false` and a `note` for the same case. `--json`/the MCP response's `released` field is the reliable way to tell a real release from a no-match. |
