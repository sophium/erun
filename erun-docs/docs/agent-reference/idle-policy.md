...
| Released by | `erun activity lease release --id <id>`, or the MCP `activity_lease_release` tool. Releasing an unknown or expired lease still succeeds (a wrapper's exit trap must never fail a job that already finished cleanly), but the CLI reports `lease released: <id>` only when a held claim actually existed and was removed, and `lease not held: <id>` otherwise — the MCP tool reports the same distinction as a `released` boolean on its result. |
...
| `activity_lease_release` | `{tenant?, environment?, id, exclusive?, scope?}` | `{tenant, environment, released, held[]}` |
...
| Who refuses what | A *job* start consults only the `environment` scope, the one that means "no other work here at all". `erun exec gate-merge` consults every claim covering the worktree it rewrites: the `worktree` scope, which claims exactly that resource and is what an exclusive take defaults to, and the `environment` scope, which covers everything here including the worktree. A claim on any other scope names a resource of the caller's own choosing (a second clone's checkout) and refuses neither. |
...
| Reported by | `erun activity lease list` renders an exclusive claim as `<name>, exclusive on <scope>, held by <holder>, expires in ...`, so a claim that refuses other work is distinguishable from plain presence — which renders as `<name>, expires in ...` with no scope or holder. Both transports read the same claim set; MCP `activity_lease_list` returns the full claim (`exclusive`, `scope`, `holder`) as structured fields rather than prose. |
...
**The `environment` scope is the one exception to "per scope, never per environment".** Everything above is about not colliding on a *resource* — a worktree, a version being released. The contention that actually invalidates results is a different thing: the pod's own CPU and memory, which no worktree boundary divides, so two claims in two clones of one repo still fight over the same cores. A claim on the scope named `environment` therefore means "no other work here at all", and it is the only scope a *job* start consults: while it is held, every `erun exec job start` in that environment is refused and told who holds it, ordinary jobs included, not only other exclusive ones. `erun exec job start --exclusive` takes exactly this claim (as `job-exclusive-<jobId>`) for the job's lifetime; `erun exec gate-merge` is refused by it, since it rewrites the shared worktree — and equally by a claim on the `worktree` scope, which is what an exclusive take records when its caller names no scope at all. An operator can take the same claim by hand (`erun activity lease take --exclusive --scope environment`) to keep an environment to themselves, with the same effect on job starts. Full contract, including the lineage exemption that keeps a gate's own nested work from being refused by its own claim: [`erun job` · Environment exclusivity](/agent-reference/cli-flags#job-exclusivity).
