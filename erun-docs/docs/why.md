...
- **The operator can join any environment, in shell or IDE.** `erun open <tenant> <env>` attaches a terminal to the runtime pod; `erun open ... --vscode` and `erun open ... --intellij` launch VS Code Remote-SSH or IntelliJ Gateway against the same environment. The operator can `git diff`, run a test, make a commit, or simply watch — and this works equally well as a primary development surface with no agent involved at all.
