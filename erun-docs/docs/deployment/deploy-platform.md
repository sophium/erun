...
- **`<tenant>-local`** — your dev workbench, **created first** (`erun init`). Here a blueprint skill scaffolds the platform's **Helm charts** (into `k8s/`) and **Terraform modules** (into the `<tenant>-terraform` module), you set the platform **deploy parameters**, and `erun-build-env` + `erun build --release` produce the custom image that carries them (plus the deploy skills). **No Cloudflare token here.** Later, once the platform is up, you set up this env's own DNS zone + TLS through the running erun API.
...
| 2 | `<tenant>-local` | skill **`erun-blueprint-platform`** | Scaffold the platform's Helm charts into `k8s/` and its Terraform modules into the `<tenant>-terraform` module. | **Planned — the missing skill** |
| 3 | `<tenant>-local` | Set the platform **deploy parameters** ([`platform:`](/reference/configuration#platform-block)) | `basedomain` (e.g. `erunpaas.com`), `acmeemail`, `authoritativeip`, `env: <tenant>-prod`. erun threads these into the PowerDNS helm values, the edge Terraform vars, and `expose`. No Cloudflare token. | Ready |
...
|---|---|---|---|
| `erun-blueprint-platform` | `<tenant>-local` | Scaffold the platform's Helm charts into `k8s/` + Terraform modules into the `<tenant>-terraform` module. | **Planned — missing** |
