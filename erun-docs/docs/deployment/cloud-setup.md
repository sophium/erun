...
The injected token is what the **`terraform-erun-cluster-edge`** module (in `erun-devops/terraform-erun/modules/`) applies to stand up the public HTTPS edge: it installs a Traefik ingress controller and cert-manager, and creates a **namespaced DNS-01 `Issuer`** (Cloudflare, using the token + the platform config's `acmeemail`) that issues a wildcard certificate for `*.<services-zone>`. The `erun-enable-hosting-edge` skill drives it end to end (apply + verify the issuer and wildcard cert). The same module owns the edge's transport policy: it redirects the plaintext entrypoint to `https` (301) and serves `Strict-Transport-Security` on the secure one, for every host the controller routes. That is where the policy belongs rather than on each Ingress — an application behind the edge cannot recover a request that arrived in cleartext, and a *relative* redirect (the hosted IdP's own login chain) inherits the scheme the browser started on, so a host-by-host fix would miss it. `hsts_max_age_seconds` defaults to one day with `includeSubdomains` and `preload` off, because HSTS cannot be recalled early; raise it once every host under the domain is verified https-only. The docs host is published to Cloudflare Pages rather than routed through the edge, so it carries HSTS in `erun-docs/static/_headers` instead. The policy is declared on the controller the module installs, so `install_ingress_controller=false` leaves it nowhere to land: that combination is **refused** while `manage_transport_policy` is left at its default `true`, instead of applying cleanly and dropping the policy. A cluster that brings its own controller sets `manage_transport_policy=false` and reads the policy the module would have carried from its `edge_transport_policy` output — the redirect and HSTS entrypoint arguments, the HSTS `Middleware` object, (with the exemption below) `redirect_objects`, and the resolved `http01_acme_challenges_present`, which is the declaration that policy was built on and is echoed there because such a caller never sees the module's own resources — to apply to that controller. **That redirect is entrypoint-wide and has no path predicate**, so it covers `/.well-known/acme-challenge/` as well: it is safe only where every certificate for every host the edge fronts is solved over DNS-01, which is true of the DNS-01 `Issuer` this module creates but not of an `Issuer` the platform brings itself. A cluster that solves any of those challenges over HTTP-01 declares it with `http01_acme_challenges_present=true`, and the module then **refuses** to produce the blanket redirect until `acme_challenge_path_exempt=true` carries it as a `Middleware` plus a catch-all router whose rule excludes the challenge prefix — without which Let's Encrypt follows the redirect to https, where the solver's Ingress has no TLS block, and the certificate fails to renew. Both default to `false`, so an edge whose challenges are all DNS-01 is unaffected. Wiring `erun expose`'s Ingress to that issuer is the remaining follow-up; the console itself is already hosted behind the edge via its own `erun-console` chart (Ingress + cert-manager annotation), configured the same way as the hosted IdP — see [Deploying the platform · The console](/deployment/deploy-platform#the-console).

**Storage reclamation on a pressured node** is a second opt-in on the same module, and it exists because the failure it prevents is a self-locking one. `local-path` is the only storage class these clusters ship, and its provisioner does both provisioning and reclamation through a short-lived helper pod that it pins onto the node holding the volume — the node under DiskPressure. kubelet's eviction manager admits and evicts by whether a pod is **critical**, and a helper pod with no `priorityClassName` resolves to priority 0: it is turned away while the node carries conditions, and evicted first when it carries any. The provisioner then waits out its 120-second create timeout and retries forever, so on that node a new PVC never binds and deleting an existing PVC frees no bytes — the reported case was a 50 GiB PVC deleted from a node at 93% used, which freed zero bytes, while the only way to free space was an out-of-band removal on the host filesystem. Passing `install_local_path_helper_pod_resilience=true` merges `priorityClassName: system-node-critical` into the distribution's own helper pod template, leaving its image and every other field intact, and states the disk-pressure toleration explicitly — a helper pod template that declares *any* tolerations suppresses the provisioner's own default one, so the two fields move together. It also annotates the provisioner Deployment's pod template with a digest of the template it wrote: the provisioner reads that template once at startup and, on k3s, has no working reload (its 30-second refresh returns early without `CONFIG_MOUNT_PATH`, which k3s's manifest does not set), so a ConfigMap write on its own applies cleanly and changes nothing until the provisioner happens to restart. Both the ConfigMap key and the annotation belong to the distribution's own manifest, which k3s re-applies on every server start — if reclamation on a pressured node stops working after a k3s upgrade, re-apply this module. It is off by default, so a module upgrade leaves an existing cluster's provisioner untouched. Two things worth knowing when diagnosing this by hand: a helper pod that cannot start shows up as `VolumeFailedDelete` on the PV and a 120-second `create process timeout` in the provisioner's log rather than as an obvious storage error, and a PV left `Released` with its directory still on disk can be restored by clearing its `claimRef` and pre-binding `spec.volumeName`, since binding is done by the PV controller and needs no helper pod.
