...
- A version is a content identity minted only by [`erun build`](/cli/build): a plain build mints `<base>-snapshot-<UTC-timestamp>`, and `--release` pins the bare semver. The env type does not decide which kind is minted.

## Release-tagged images

[`erun release`](/cli/release) itself never builds or publishes anything — it is version paperwork only. `erun build --release` builds release-tagged images for both `linux/amd64` and `linux/arm64`, then hands the version to [`erun push`](/cli/push), which pushes the per-arch tags and assembles a manifest list so `<image>:<version>` resolves to either arch automatically. `build --release` does not push directly — it reuses `push` for every publish, so a released version and a pushed snapshot are published the same way.

A build that cannot publish is not corrupt: the release tag already exists (created by `erun release`'s own stamp/commit/tag step, which `build --release` runs first) and is a permanent, honest record of the source that was attempted — fix the issue and run `erun build --release` again.
...
2. `erun build --release` reads `<projectroot>/<tenant>-devops/VERSION`, syncs the chart `version` / `appVersion`, creates the release commit and a local tag, pushes the tag, syncs packaging checksums, and advances the next patch on `release.developbranch` — all `erun release`'s own steps — then builds the multi-arch images, runs `push` at the release version (per-arch tags + manifest list + the runtime chart), and verifies each published manifest resolves.
...
`release.mainbranch` does not have to stand still for the duration of a release. `erun build --release` re-reads it just before the build and refuses if it moved, and its final push rebases onto a branch that moved while the release's own git steps were running — so a pull request merging mid-release costs seconds or nothing, never a published version the repository has no commits for. See [Release version policy · Lifecycle algorithm](/agent-reference/release-policy#lifecycle-algorithm).
