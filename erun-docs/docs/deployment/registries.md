...
A deploy running **inside an environment's own runtime pod** has one further source, consulted only after all three routes above come up empty: the `dockerconfigjson` Secret that environment already declared as its [`imagepullsecrets`](/reference/configuration#advanced-image-pull-secrets). That is the credential the pod itself pulls its images with, and its service account can read it, so the deploy reads it rather than probing anonymously. The distinction matters because an anonymous read of a private chart is not "not published" — it is an unanswered question, and `erun deploy` refuses to guess at one. A private chart therefore resolves in the pod without anyone logging in there first.
