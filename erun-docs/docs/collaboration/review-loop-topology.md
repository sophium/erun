...
`erun-builder` and `erun-reviewer` are the two standing roles this topology names, shipped as reusable agents — see the [reusable-agent artifact spec](/agent-reference/agents-spec) for their catalogue entries, and [Agent patterns § 5](/collaboration/agent-patterns#5-review--comment--fix-loop) for the underlying API calls. The [`/erun-merge`](/agent-reference/skills-spec#erun-merge) and [`/erun-review`](/agent-reference/skills-spec#erun-review) skills these two agents drive are documented in the [skill catalogue](/agent-reference/skills-spec#built-in-skill-catalogue). Assigning a reviewer is available from every erun client: [`erun review reviewers add`/`list`/`remove`](/cli/review#review-reviewers) on the CLI, the `review_reviewers_add`/`review_reviewers_list`/`review_reviewers_remove` MCP tools, and the desktop app's review **Reviewers** surface. A reviewer is named by enrolled user id — `erun platform user list` prints the tenant's — and once assigned, `erun review list --waiting-on-me` returns that review for them. See [Reviews § Author, reviewers, and discovery](/collaboration/reviews#author-reviewers-and-discovery) for the resource, and [merge queue](/collaboration/merge-queue) for what actually gates a merge (assignment gates none).
