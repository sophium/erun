...
A review created before the platform recorded a repository carries none. It appears in an unfiltered listing — the union across repositories — and a promotion that names no repository is refused with `MERGE_QUEUE_AMBIGUOUS` whenever that union holds more than one repository's reviews, rather than promoting a head that names no single repository. Naming one is how a mixed queue is resolved.
...
| No repository was named and the queue holds more than one repository's reviews | `409 Conflict` | `{code: "MERGE_QUEUE_AMBIGUOUS", message, details}` — `details` names `targetBranch` and the `repositories` | Name one: `erun review queue advance --repository <remote> --target-branch <branch>`. |
