...
- **List every identity in the platform's own organization**, with its current state — or, pointed at another tenant via the [scope selector](/collaboration/cross-tenant-administration#administering-users), that tenant's own organization's identities instead.
