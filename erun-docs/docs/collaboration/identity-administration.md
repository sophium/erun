...
- **Enroll a user.** Give a username and email; the console creates the identity in the platform's IdP and the matching erun user in one action. If the erun-side half fails after the identity provider half succeeds, the console tells you the identity provider id it created so you can retry the mapping rather than enrolling a duplicate.
  ...
  - **A person enrolled into the wrong tenant's organization cannot be moved there afterward.** The identity provider has no way to reassign an existing identity to a different organization — recovering means deactivating the misplaced identity and enrolling them again with the right tenant chosen. Point the [scope selector](/collaboration/cross-tenant-administration#administering-users) at that tenant first — the Users view then lists and deactivates that tenant's own organization's identities directly, rather than the underlying API.
- **List every identity in the platform's own organization**, with its current state — or, pointed at another tenant via the [scope selector](/collaboration/cross-tenant-administration#administering-users), that tenant's own organization's identities instead. A row that is also an enrolled erun user shows a **You** label when it is your own — matched against your own signed-in identity, not against a name — and shows your erun username (the same one in the header and in every review, role, and audit entry) alongside the identity provider's own username whenever the two differ, so you can always recognize your own row and know which name to use where. Every row also shows its OIDC subject, the one value that reliably identifies the same person across both directories.
