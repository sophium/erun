...
| `404` | — (generic) | The review id or the `environmentId` doesn't exist or isn't visible to the caller's tenant. Both build routes confirm an `environmentId` names one of the caller's own environments before accepting it, and answer an id belonging to another tenant exactly as they answer one that exists nowhere — so a refusal never reports whether an id exists somewhere the caller cannot look. |
...
Retention for the tenant-wide build history is intentionally not part of this contract yet: the table has no TTL or row cap today, the same as every other high-volume table on the platform (`usage_events`, `gate_runs`, `audit_events`). See [Data retention](/deployment/data-retention) for the tables the platform's own sweep does cover.
