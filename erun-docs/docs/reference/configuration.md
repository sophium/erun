...
| `environments.<env>.docker.platforms` | list | `erun build`, `erun push` | Pins the `docker --platform` targets for a non-release build/push in this env (e.g. `[linux/amd64]`), for a cluster that can only ever run one architecture. `--platform` on the command line overrides it for one invocation. Never applies to `erun build --release`, which always builds every platform erun supports. See [Multi-architecture](/cli/build#multi-architecture). |
