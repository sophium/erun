...
| `runtimedindpod.cpu` | string | helm chart (`runtime.dind.resources.limits.cpu`) | CPU limit for the `erun-dind` sidecar — the container that actually runs `erun build`/`erun release`, independent of `runtimepod.cpu`. When unset, a deploy derives it from the machine it runs on: the node's core count less two cores of headroom for the runtime container and system work, never below one (so a 24-core node gives the sidecar `22`). Set with `erun init --dind-cpu` / `erun resize --dind-cpu`. |
