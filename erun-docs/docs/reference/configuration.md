...
| `orchestrators[].id` | string | `erun list`, `erun orchestrator` | The stable id every command and MCP tool addressing this orchestrator uses. ERun derives it from the name and never rewrites it, so a rename does not strand the orchestrator's conversations, claim label, or activity history. |
| `orchestrators[].name` | string | `erun list`, the desktop sidebar | Display name, yours to edit. |
| `orchestrators[].alias` | string | `erun list`, `erun orchestrator set-alias`, the desktop's Edit orchestrator dialog | The erun platform alias this orchestrator declares as its own: the orchestrator-scoped counterpart of a tenant's `primarycloudprovideralias`. Absent means it declares none. It selects among aliases the host already has — it is not a credential, and nothing here creates or signs in to one — so a value this machine cannot resolve is refused rather than stored. **Declaring, validating, and reading back is all this field does today.** No orchestrator command, and no orchestrator session, resolves through it: the session is launched with `ERUN_ORCHESTRATOR_ID` and no alias variable (see [Environment variables · Orchestrator-session variables](/reference/env-vars#orchestrator-session-variables)), and an orchestrator's platform calls still resolve **this machine's own** alias whatever it declares. Giving an orchestrator an identity of its own, and mapping aliases per linked environment, is `(Planned.)` and stays open in [erun#2684](https://github.com/sophium/erun/issues/2684). `erun orchestrator set-alias` sets it from the terminal, the desktop dialog has a **Platform alias** control, and `erun list` prints it as `platform alias:`. |
| `orchestrators[].environments[]` | list | `erun list`, the desktop Edit orchestrator dialog | The environments this orchestrator links, each with the host directory it reviews it in and the role it uses it for. |
| `orchestrators[].environments[].role` | string | `erun list`, `erun orchestrator set-role`, the desktop dialog | `code`, `build`, or `runtime`; absent means undeclared. See [`erun orchestrator`](/cli/orchestrator). |
| `orchestrators[].directories[]` | list | `erun list`, the desktop Edit orchestrator dialog | Directories this orchestrator works in that belong to no environment, each named by path alone. A directory is a complete definition on its own. |
...
| `runtimedindpod.cpu` | string | helm chart (`runtime.dind.resources.limits.cpu`) | CPU limit for the `erun-dind` sidecar — the container that actually runs `erun build`/`erun release`, independent of `runtimepod.cpu`. Default `12`, which is a sizing rule rather than a fixed number: the node's CPUs divided across the build-capable environments erun expects to be building on it at once, floored at `4` (see [Sizing the build CPU cap](/concepts/runtime-pods#sizing-the-build-cpu-cap)). A limit is a ceiling, not a reservation, so size it for the node — a cap well under the node throttles the build while the node sits idle. Set with `erun init --dind-cpu` / `erun resize --dind-cpu`. |
...
| `hosted` | record | `erun platform env push`/`pull`, `erun list`, desktop Manage → General | The platform row this local environment corresponds to, when it has one: `apihost`, `tenantid`, `environmentid`, and `definitionrevision` (the revision this machine last synced from; absent or `0` means never synced). Written when `erun platform env register --adopt --definition` uploads the environment's portable settings, and re-stamped by every later `push`/`pull`. It is a purely local fact — the platform stores no equivalent — and it is **host-owned**: it never travels in a definition transfer. A `push` or `pull` whose resolved row disagrees with it on the api host, tenant or environment id is refused rather than re-pointed. See [Managing hosted environments · The hosted marker](/collaboration/hosted-environments#the-hosted-marker). |
...
| `docker.platforms` | list | `erun build` | Project-wide default for the `docker --platform` targets of a local build (e.g. `[linux/amd64]`), inherited by every environment that declares no `platforms` of its own — for a project whose machines can only ever run one architecture. Declare it once here instead of repeating it per environment, which silently leaves any environment nobody listed on the multi-arch build. It narrows only what a local build mints: publishing never consults it, because `erun build --release` and `erun push` both resolve every platform erun supports, so a published version always carries all of them. See [Multi-architecture](/cli/build#multi-architecture). |
| `environments.<env>.docker.platforms` | list | `erun build` | Pins the `docker --platform` targets for a local build in this env (e.g. `[linux/amd64]`), overriding the project-wide `docker.platforms` default. An explicit empty list (`platforms: []`) opts this env out of that default and keeps the multi-arch build — the escape hatch for a generic env name like `local` that can belong to a machine of any architecture. `--platform` on the command line overrides both for one invocation. Never applies to `erun build --release` or `erun push`, which always resolve every platform erun supports. See [Multi-architecture](/cli/build#multi-architecture). |
...
erun manages only the runtime pod's and `erun-dind` sidecar's resource **limits**: `EnvConfig.runtimepod.cpu`/`.memory` and `EnvConfig.runtimedindpod.cpu`/`.memory` (set with `erun init --runtime-cpu`/`--runtime-memory`/`--dind-cpu`/`--dind-memory`, `erun resize`, or the desktop's env settings) are always `--set` as `runtime.resources.limits.{cpu,memory}` and `runtime.dind.resources.limits.{cpu,memory}` respectively. The requests are overlay-only for both containers:
...
| `runtime.dind.resources.requests.cpu` | `0.25` | CPU request for the `erun-dind` sidecar. |
| `runtime.dind.resources.requests.memory` | `1024Mi` | Memory request for the `erun-dind` sidecar. |
...
- Today's promoted operations:
  ...
  | `kubectl-namespace-get` | `kubectl get namespace <name> -o name`, the existence check `erun deploy` runs ahead of nearly every helm install so it only creates a namespace that is actually missing. | Every other `kubectl` call site (the non-Secret `apply`s) — still subprocess-only. |
  | `kubectl-pvc-get` | `kubectl get pvc <claim> -o name`, the existence check `erun deploy` runs ahead of a runtime-release install to decide whether its dedicated worktree volume already exists (and, if not, to announce the adoption from the home volume). | Every other `kubectl` call site (the non-Secret `apply`s) — still subprocess-only. |
  | `kubectl-secret-get` | `kubectl get secret <name> -o json`, the read `erun deploy` runs ahead of refreshing an image-pull Secret so this run's resolved registry credentials merge into its existing coverage instead of replacing it outright. | Nothing else — the Secret's own `apply` (the write this read feeds) is `kubectl-secret-apply` below. |
  ...
  | `kubectl-deployment-wait` | `kubectl wait --for=condition=Available --timeout <t> deployment/<name>`, the rollout wait `erun open` runs ahead of its shell exec, `erun doctor` runs ahead of every action, and waking a stopped environment runs after scaling it back up. The library path polls the same condition on an interval rather than opening a real watch. | The non-Secret `apply`s — still subprocess-only. |
  | `kubectl-secret-apply` | `kubectl apply -f -` of a core/v1 Secret: the image-pull Secret `erun deploy` refreshes before every rollout, the registry-credential Secret `erun init` provisions, the Cloudflare token and desktop MCP public key a deploy delivers, and the DNS-01 broker token `erun expose` applies. The library path reproduces kubectl's **client-side** apply — the `last-applied-configuration` annotation plus a three-way strategic merge patch, under kubectl's own field manager — rather than a server-side apply, which would record ownership differently and leave a mode flip permanently visible in the object. | Every other `apply`: the namespace `ResourceQuota`/`LimitRange` pair (one two-document manifest, two kinds), and `erun expose`'s `Ingress`, cert-manager `Issuer` and `Certificate` (a custom resource has no typed patch metadata, so kubectl merges it by a different algorithm). |
