...
| `runtimepod.memory` | string | helm chart (`runtime.resources.limits.memory`) | Memory limit (e.g. `12288Mi`, `2Gi`). Default `12288Mi`: agents run `make check-gate` in this container, which is the same ten-target gate the `erun-dind` sidecar runs during an image build (see the row below). Trim it for an env that only serves an app; raise it with `erun resize --memory` if the cgroup reports OOM kills. |
