...
| `openrouter.authtokensecret` | string | `erun deploy`, the in-pod runtime | Name of a Kubernetes Secret, in each environment's own namespace, holding the gateway credential. **Optional:** the catalog is erun-level, so there is one Secret name for every environment, and leaving this empty resolves to `erun-claude-gateway`. Name it only to reuse a Secret you already manage. It is a reference and never the token itself, so this file stays safe to back up and share; the operator provisions the Secret by whatever means they already use for secrets in the cluster. |
...
| `claude.gatewayauthtokensecret` | string | chart (`claude.openRouterAuthTokenSecret`) | A Secret in this environment's namespace to read the gateway credential from, instead of the catalog's own `openrouter.authtokensecret`. For a credential managed separately, such as a per-tenant one. |
