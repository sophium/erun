...
| `orchestrators[].alias` | string | `erun list`, `erun orchestrator set-alias`, the desktop's Edit orchestrator dialog | The erun platform alias this orchestrator declares as its own: which configured erun-type alias it acts as, and the orchestrator-scoped counterpart of a tenant's `primarycloudprovideralias`. Absent means it declares none and follows this machine's own alias. It selects among aliases the host already has — it is not a credential, and nothing here creates or signs in to one — so a value this machine cannot resolve is refused rather than stored. `erun orchestrator set-alias` sets it from the terminal, the desktop dialog has a **Platform alias** control, and `erun list` prints it as `platform alias:`. |
