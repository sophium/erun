...
| `ANTHROPIC_AUTH_TOKEN` | string | unset | The gateway credential, which Claude Code sends as `Authorization: Bearer`. Present only when the catalog names a Secret; the value comes from that Secret, never from config, so no token reaches helm argv or a launch command. | Root config `openrouter.authtokensecret` and `openrouter.authtokenkey`. |
| `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY` | bool | unset | Ask the gateway for its model list at startup and add those names to the `/model` picker alongside the built-in entries. Set whenever a gateway is configured. | Root config `openrouter`. |
