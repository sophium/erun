...
| `erun-orchestrate` | Workflow | Act as a host-side orchestrator across agent environments of any type: drive each pod-backed env through its erun MCP (its raw/build/deploy tools, or its in-pod agent), review that env's host directory read-only — a synced mirror for a `remote-agent` env, the worktree itself for a `local-agent` one — and work a `host` env in its own directory directly, since it has no pod and no in-pod agent to delegate to; run built artifacts on this machine, never editing a pod-owned review directory or reaching into the pod with kubectl ([spec](/agent-reference/skills-spec#erun-orchestrate)). |
