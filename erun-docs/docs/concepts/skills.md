...
| `erun-orchestrate` | Workflow | Act as a host-side orchestrator across remote-agent environments: delegate work to the in-pod agents, review each env's synced mirror read-only, and run built artifacts on this machine — never editing the mirror directly ([spec](/agent-reference/skills-spec#erun-orchestrate)). |
