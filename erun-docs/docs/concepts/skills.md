...
| `erun-merge-queue-drive` | Workflow | Drive a review already promoted to `MERGE` to completion: build the prospective squash merge, gate it with a real `erun build`, and push and report `MERGED` only on green. Not automatic yet — invoked explicitly against a review the queue already promoted, never advances or overrides the queue itself ([spec](/agent-reference/skills-spec#erun-merge-queue-drive)). |
