...
| `erun-onboard-service` | Workflow | Adopt a repo that already has its own layout — point erun at the Dockerfiles and charts where they already are, preflight the environment, then build, deploy, and serve the service over HTTPS with a verified certificate ([spec](/agent-reference/skills-spec#erun-onboard-service)). |
...
| `erun-orchestrate` | Workflow | Act as a host-side orchestrator across agent environments of any type: drive each pod-backed env through its erun MCP (its raw/build/deploy tools, or its in-pod agent), review that env's host directory read-only — a synced mirror for a `remote-agent` env, the worktree itself for a `local-agent` one — work a `host` env in its own directory directly, since it has no pod and no in-pod agent to delegate to, and work the directories it names for itself, which belong to no environment at all; run built artifacts on this machine, never editing a pod-owned review directory or reaching into the pod with kubectl ([spec](/agent-reference/skills-spec#erun-orchestrate)). |
...
| `erun-review` | Workflow | Review someone else's branch on the erun platform: read the diff, comment only on what should block the merge, and push a proposal branch where there's a concrete fix. Never advances or overrides the merge queue, closes the review, or resolves a thread it didn't open ([spec](/agent-reference/skills-spec#erun-review)). |
| `erun-merge-queue-drive` | Workflow | Drive one or more reviews already promoted to `MERGE` to completion: batch their sources into one prospective merge (skipping any that conflict), gate the landed stack with a real `erun build`, and push and report `MERGED` per review only on green. Not automatic yet — invoked explicitly against reviews the queue already promoted, never advances or overrides the queue itself ([spec](/agent-reference/skills-spec#erun-merge-queue-drive)). |
