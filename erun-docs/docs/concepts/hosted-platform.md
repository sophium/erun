...
**When the fallback is masking a misnamed platform tenant.** This fallback is correct whenever a tenant genuinely has no image of its own — but the platform's own `OPERATIONS` tenant is a special case: if it bootstrapped under a name other than its declared `ERUN_TENANT` (see [First-identity bootstrap](/agent-reference/api-protocol#tenant-issuers)), its environments resolve `<legacy-name>-devops`, confirm it missing, and fall back to the canonical image — even when the platform has actually published `<ERUN_TENANT>-devops` and simply never resolves it under the name it is actually running. The control plane checks for exactly that case: when the fallback fires for the `OPERATIONS` tenant and a differently-named image under `ERUN_TENANT` is affirmatively confirmed present, the API log names the substitution explicitly (`runtime image fallback substitution: operations tenant "<legacy-name>" has no own image … but ERUN_TENANT="<declared>" has a published <declared>-devops`) rather than silently substituting the canonical image. Seeing that line is the signal to reconcile the tenant's name — see [`PATCH /v1/tenants/reconcile-bootstrap-name`](/agent-reference/api-protocol#patch-v1tenantsreconcile-bootstrap-name).
...
**Requires the cluster's DNS-01 webhook shim.** The per-env `Issuer` this provisions forwards its ACME challenges through the cluster-wide cert-manager webhook shim (`terraform-erun-cluster-edge`'s `install_dns01_webhook`, on by default when the module's `dns01_provider` is `"powerdns-broker"` but installable independently — a platform can keep its own wildcard Issuer on `"cloudflare"` or `"powerdns-rfc2136"` while still installing the shim for per-tenant Issuers like this one) — a one-time, per-cluster Terraform apply, not something a per-env deploy can install for itself. Without that shim already running, the `Issuer` this creates never becomes `Ready` and the certificate never issues; the environment stays reachable over http (or the ingress controller's default self-signed certificate for https) until the shim is installed.
