...
A version normally comes straight from config, but a tenant that ships its own runtime image under its own tag scheme (rather than erun's stock `erun-devops` image) has no version erun can parse out of that config at all. When that happens, `list` falls back to asking the environment itself: it probes that environment's own local MCP edge — the same endpoint [`erun mcp call`](/cli/mcp) and `erun open`'s port-forward use — for the erun version it's actually running. An environment nobody has opened (or one that's genuinely stopped) still reports `version=none`, same as before; an environment that *is* up but still reports no version prints `[up but version unresolved]` instead, since an environment that's reachable and unreadable is itself worth flagging.

Like the rest of `list`, this report exits `0` on its own — `list` reports, it doesn't gate. Add `--fail-on-drift` to make that one invocation exit non-zero when an environment is behind the tenant's max, is up but its version couldn't be determined even via the live probe, or the named gate environment is itself behind (or its version couldn't be resolved), so a script or a schedule can act on it:
...
| `--fail-on-drift` set and the report finds drift (an environment behind max, an environment that is up but its version couldn't be determined even via the live MCP-edge probe, a behind gate, an unreachable/behind/ahead plane or console, or an unresolved published baseline). | The full report still prints, then the command exits non-zero naming what it found. Never fires under `--dry-run` — nothing was probed, so there is nothing to fail on. An environment nobody has opened (or one that's genuinely stopped) is not a finding on its own — only one that answers the probe and still can't report a version is. |
