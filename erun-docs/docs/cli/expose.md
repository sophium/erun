...
**When the Service is not named that way.** That derivation is right for a chart erun scaffolded and wrong for a repo that brought its own, whose chart names its Service itself. Routing to a derived name that does not exist produces the worst shape of failure: a hostname that resolves and an ingress that 503s. Pass `--backend-service <name>` to route to the real one, and keep `<service>` as the public label; the derivation stays the default for everyone else. [`erun services`](/cli/services) lists the Services an environment runs and flags the ones already exposed, so the name to route to is one read away — the same list the desktop's Ports tab offers as a picker.
...
2. **Host-routing Ingress.** It applies an Ingress named `expose-<service>` into the env's namespace, routing the hostname to the Service `--backend-service` names — or, when that is unset, to the tenant-scoped Service `<tenant>-<service>` — on `--port` (default `80`).
...
| `--backend-service <name>` | The in-namespace Service the Ingress routes to, when it is not `<tenant>-<service>`. [`erun services`](/cli/services) lists the candidates. |
