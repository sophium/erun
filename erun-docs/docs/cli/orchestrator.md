...
Set what an orchestrator uses one of its already-linked environments for: a **code** environment writes code and iterates fast; a **build** environment checks out pushed branches, runs the gates, and cuts releases. See [`erun list`](/cli/list) for the same value read back, and [Agent reference · Skills spec](/agent-reference/skills-spec#erun-orchestrate) for how the orchestrator itself uses the role.

### Synopsis

```
erun orchestrator set-role ORCHESTRATOR_ID TENANT ENVIRONMENT --role <code|build|none> [flags]
```
...
| Flag | Description |
|---|---|
| `--role` | Required. `code`, `build`, or `none` to declare the role undeclared again. |
...
| `--role` is not `code`, `build`, or `none`. | Aborts with `invalid role "<value>": must be "code", "build", or "none" (undeclared)`; exit code 1; nothing is written. |
