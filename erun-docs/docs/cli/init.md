...
Advanced flags (`--project-root`, `--remote`, `--no-git`, `--version`, `--runtime-image`, `--runtime-cpu`, `--runtime-memory`, `--codecommit-ssh-key-id`, `--confirm-environment`) and the full lifecycle algorithm are on [Agent reference · CLI flag spec](/agent-reference/cli-flags#erun-init). Common root flags (`--dry-run`, `-v`/`-vv`, `--time`) apply.
