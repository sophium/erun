...
Advanced flags (`--project-root`, `--no-git`, `--version`, `--runtime-cpu`, `--runtime-memory`, `--dind-cpu`, `--dind-memory`, `--codecommit-ssh-key-id`, `--confirm-environment`) and the full lifecycle algorithm are on [Agent reference · CLI flag spec](/agent-reference/cli-flags#erun-init). `--dind-cpu`/`--dind-memory` size the `erun-dind` sidecar — the container that actually runs `erun build`/`erun release` — independent of `--runtime-cpu`/`--runtime-memory`; raise `--dind-memory` if a multi-arch release OOMs there (see [`erun resize`](/cli/resize) to change it on an environment that already exists, without a full `init`). `--dind-cpu` pins the sidecar's CPU limit; leave it off and each deploy derives that limit from the machine it runs on — the node's cores less two cores of headroom, never below one — so a build is never throttled to a fixed number while the node sits idle. Name it explicitly when the deploy runs somewhere other than the node, such as a workstation driving a remote cluster. `--remote` is a deprecated alias for `--type=remote-agent`. `--bootstrap` is deprecated and ignored — `init` no longer scaffolds a `<tenant>-devops/` module; environments deploy the published `erun-devops` chart directly, and passing the flag only prints a deprecation warning. Common root flags (`--dry-run`, `-v`/`-vv`, `--time`) apply.
