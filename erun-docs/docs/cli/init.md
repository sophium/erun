...
Advanced flags (`--project-root`, `--no-git`, `--version`, `--runtime-cpu`, `--runtime-memory`, `--dind-cpu`, `--dind-memory`, `--codecommit-ssh-key-id`, `--confirm-environment`) and the full lifecycle algorithm are on [Agent reference · CLI flag spec](/agent-reference/cli-flags#erun-init). `--dind-cpu`/`--dind-memory` size the `erun-dind` sidecar — the container that actually runs `erun build`/`erun release` — independent of `--runtime-cpu`/`--runtime-memory`; raise `--dind-memory` if a multi-arch release OOMs there (see [`erun resize`](/cli/resize) to change it on an environment that already exists, without a full `init`). `--remote` is a deprecated alias for `--type=remote-agent`. `--bootstrap` is deprecated and ignored — `init` no longer scaffolds a `<tenant>-devops/` module; environments deploy the published `erun-devops` chart directly, and passing the flag only prints a deprecation warning. Common root flags (`--dry-run`, `-v`/`-vv`, `--time`) apply.
