...
- On current `main`, both `TestDesktopSurfaceGate` and `TestNoUnboundAppMethods` are clean: whip's desktop control gave `whip` a real reference in `erun-ui/frontend/src`, and `whipAllOrchestratorsNow` is called from `WhipNow` (`erun-ui/whip.go`).
