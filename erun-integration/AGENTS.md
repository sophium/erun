...
- **GitHub REST wire calls with no integration-suite seam**: `postGitHubCommitStatus` (`erun-common/report_commit_status.go`) and `listOpenGitHubPullRequests`/`postGitHubIssueComment`/`closeGitHubPullRequest` (`erun-common/close_pull_request.go`), plus `ClosePullRequestHeadMovedError.Error` (only ever called when a real, network-returned head mismatch is found). The harness has no seam to point `githubAPIBaseURL` at a local server the way the chart-registry probe below does, so validation, dry-run tracing, remote-url parsing, and the no-token refusal are exercised end-to-end from the binary in `exec_test.go`'s `report_commit_status_*`/`close_pr_*` scenarios, while the actual GitHub wire shape and failure-response surfacing are covered only by each file's own `erun-common` unit tests (`report_commit_status_test.go`, `close_pull_request_test.go`) against a real `httptest` server. `COVERAGE_THRESHOLD`'s default moved from `75.6` to `75.3` in the same commit that added `close_pull_request.go`'s network functions, the tracked discussion this section's own rule asks for.
