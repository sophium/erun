...
- **A cited coverage baseline is a claim about the environment it was measured in, not a fact about the commit — re-measure cleanly before trusting it (#2191).** Issue #2191 and its companion bisection PR (#2192) cited `main` at `534fd783` as measuring 72.6%, "verified reproducible" over 8 runs. Re-measuring that exact commit in a clean, single-tenant, contention-free pod (`./scripts/integration-test.sh` run twice, zero test failures, zero `t.Skip`s — confirmed with `go test -v -run TestOpen` showing all 76 subtests PASS) gave **75.7%** both times — already above the 75.1% threshold. The per-file uncovered-statement breakdown for `erun-common/deploy.go` (380/1769), `release.go` (207/916), `cloud_context.go` (201/1002), and `init.go` (191/831) matched the issue's own cited numbers exactly, digit for digit, ruling out a different commit or a dirty tree as the explanation. The only files that disagreed were `open.go` (133 uncovered here vs. 277 cited) and `cmd/open_ide.go` (86 vs. 258) — both belong to `TestOpen`, the one top-level test this file's own "Parallel execution" section already documents as flaky under contention (a goroutine-leak timeout, plus the port-forward-simulator hazards `skipIfPortsBusy` guards against). The likely explanation is root `AGENTS.md`'s "A Gate Holds The Environment To Itself": whatever environment measured 72.6% was contended (concurrent jobs, busy ports, or CPU throttling), which silently skipped or truncated `TestOpen`'s real-run scenarios without failing the overall `go test` run — "8 clean runs" meant no hard failures, not zero skips. **The lesson, not a one-time correction:** before spending effort chasing a cited coverage gap, re-measure the cited commit yourself in an exclusive, uncontended environment (`erun exec job start --exclusive` per root `AGENTS.md`, or a pod with nothing else scheduled) and diff the per-file breakdown against what was cited — a match on most files and a divergence isolated to `TestOpen`/`open_ide.go` is this same failure mode recurring, not a new regression.
...
- **A fifth source, at a finer granularity than the four above: every flag on an operator-facing command.** The four enumerations all reason about whole units — a CLI path, an MCP tool, a route, a config field — so the gate asked "does `erun review list` have a desktop surface?" and never "does each thing `erun review list` can now do have one?". A capability delivered as **new flags on a command that already has a desktop surface** therefore passed unexamined: review discovery grew seven CLI filters (`--mine`, `--waiting-on-me`, `--author-user-id`, `--reviewer-user-id`, `--status`, `--source-branch`, `--target-branch`) while the gate kept clearing `review list` on the command's own name alone (#2141). `cliFlagCapabilities` (`desktop_surface_test.go`) closes that dimension: one `Capability` per own (non-inherited, non-`Hidden`) flag of every command the gate already holds to needing an operator entry point — runnable, not `Hidden`/`Deprecated`, and not declared agent-facing on either the MCP or the CLI side. A flag on a command nobody is expected to reach from the desktop needs no affordance either, so those are skipped with their command. An inherited persistent flag belongs to the ancestor that registered it and is audited there once, not re-audited per descendant. The tree walked is `auditCommandTree()`'s, not `cmd.CommandTreeForAudit()`'s directly, so `build`/`push`'s flags enumerate identically regardless of the cwd the test binary runs from — see the CLI help drift gate's section below for why those two commands are cwd-conditional.
- A flag matches by `Capability.Tokens`, an **any-of** list of the two spellings the same dimension legitimately appears under on the frontend side: the kebab-case flag name a call site can pass verbatim (`waiting-on-me`) and the camelCase field a request model names it by (`waitingOnMe`). Either is a real way in. Like `Token`, this is deliberately loose, and it is loose in the safe direction only for *some* flags: a short flag name that is also an ordinary English word (`--status`, `--source-branch`) will clear on unrelated UI copy — `sourceBranch` is a display column on the desktop's Reviews tab with no filter behind it — so the flag dimension catches a distinctly-named capability reliably and a generically-named one only sometimes. That is the same "false positives make the gate too lenient, never too strict" trade the CLI/MCP `Token` precedent already takes; the alternative (hand-classifying all 378 flags as surfaced/not) would assert an unverified fact per entry, which is the false confidence this gate exists to prevent.
- A flag opts out through **two lists in `erun-cli/cmd/command_tree.go`, keyed `"<command path> --<flag>"`**, which are the flag-granularity twins of `InternalAPIRoutes` and `KnownUnsurfacedRoutes` and must not be confused with each other:
  - `cliOnlyAgentFacingFlags` asserts a design decision that holds indefinitely: the flag is structurally about the CLI's own invocation rather than about the operation, so no desktop affordance could correspond to it. The 15 entries today are prompt bypasses (`--confirm-environment`), local shell integration (`open --no-shell`, `--no-alias-prompt`), the offline `version --no-registry`, the exit-code-only `list --fail-on-drift` (see `erun-cli/AGENTS.md` § "Exit-Code Contract"), the scripted-composition switches `expose`/`unexpose` document in their own usage strings (`--skip-if-unconfigured`, `--platform-namespace`), and doctor's in-pod-only repairs (`--finish-remote-init`, `--sync-config`, and the two value flags feeding them) — the desktop app runs on the operator's machine, never in the runtime pod, so the surface that would host those does not exist.
  - `knownUnsurfacedFlags` is the shrink-only baseline, on exactly the terms `KnownUnsurfacedRoutes`'s own subsection below sets out: a record of gaps that still exist, **not** a second, softer opt-out, and never the place to send a fresh gate failure. It started at the 40 flags with no operator-surface reference the day the gate learned to see flags — build cache bypass, cloud-provider and EC2 enrollment inputs, MCP-edge auth and rollout patience on `deploy`/`init`, doctor's prune/repair/restore actions, job lease and output caps, the whole `expose` TLS/ingress story, `list`'s control-plane and gate-environment reports, and platform-provision root-disk shape. `FindStaleBaselineEntries` covers these the same as routes, so a flag that gains an affordance without its entry being deleted fails the gate.
  - `TestCLIFlagDeclarationsNameRealFlags` fails on a key in either list that no longer names a registered flag on an operator-facing command, so a renamed or deleted flag cannot leave an exemption standing that silently covers nothing — the same staleness discipline `buildCheckCoverage` applies to its own entries.
- **The re-check #2141 asked for, recorded because it corrects the issue's own reading.** #2141 reported the desktop Reviews tab as *strongly indicated but unconfirmed* to have no filter affordance at all (the reporter could not run the desktop app; the published docs' silence was the only evidence). Reading `erun-ui/frontend/src/components/app/TenantDashboardPanels.Reviews.tsx` against the CLI's seven filters: two of them — `--mine` and `--waiting-on-me` — do have real affordances (`ReviewFilterSegmentedControl`, with counts, backed by `reviewFilter.mine`/`reviewFilter.waitingOnMe` in `app/tenantDialogThunks.ts`), so the docs' silence was indeed not proof. The other five (`--author-user-id`, `--reviewer-user-id`, `--status`, `--source-branch`, `--target-branch`) have no filter control. **The flag dimension does not catch those five, and that is the honest limit of this mechanism, not an oversight:** every one of their tokens already appears in the same tree as a *display* field — `authorUserId` renders a row's author, `sourceBranch`/`targetBranch` render the branch arrow — so a substring match clears them for the wrong reason, exactly the "roles" word-collision the API-route `Pattern` work hit from the other direction. Closing those five means giving them affordances, which no gate can do; what the gate now guarantees is that a *distinctly-named* dimension cannot ship unexamined.
- **The finer granularity costs nothing in gate runtime; it saved some.** Measured on this repo, `TestDesktopSurfaceGate` went from ~2.71s to ~1.90s (3 runs each) while the audited capability count grew by 378. The reason is that `FrontendSource.Contains` used to `strings.ToLower` the whole ~2.7 MB concatenated operator surface once *per capability*; the classifier now lowercases it once per audit (`preparedSource`) and every check reads that. Adding capabilities to this gate is close to free, so a future source should be judged on whether its signal is real, not on what it costs to run.
...
- **Available work, not yet closed**: two clusters triaged for #2191 that are genuinely CLI-reachable but need new stub infrastructure this pass didn't build. Neither is a structural gap — both are candidates for a future PR's scenarios, not this file's exemption list. (1) `deploymentMatchesExpectedSettings` and its helpers (`matchesExpectedRepoPath`, `matchesExpectedBool`, `matchesExpectedPort`, `matchesExpectedRuntimePod`, `deploymentExpectedMatches.apply`/`.ok`, `erun-common/deploy.go`) run only when `open`'s `shouldDeployRuntime` finds an existing deployment (`kubectl get deployment ... -o name` succeeds) *and* at least one `Expected*` field is set, at which point it issues a second `kubectl get deployment ... -o json` call and parses the pod's env vars to decide whether a redeploy is needed. No existing `open_test.go`/`deploy_test.go` scenario stubs this second, JSON-shaped call — reaching it needs a stub that answers differently for `-o name` vs `-o json` on the same command, which this suite doesn't have a shared helper for yet (see "Inlining stubs that branch on argv via shell `case`" under Anti-patterns below — factor it into `internal/fixture` rather than inlining it once someone builds it). (2) `resolveRunningRuntimeVersion`/`ResolveDeployedHelmReleaseVersion` (`erun-common/deploy.go`) — the `helm get metadata` read that heals a cached (`SkipHelm`) deploy's persisted runtime version to what the release is actually running — is currently owned by `erun-cli/cmd/open_persist_runtime_version_test.go`'s white-box unit test, whose own comment states "this side effect is unreachable from the dry-run integration binary." `SkipHelm` is set only by the orchestrator that resolves a cached, fingerprint-promoted deploy spec (no images pushed); simulating that from the integration harness needs real build/push fingerprint-cache scaffolding, not a small stub addition, so this stays with the unit test for now rather than being force-fit into a scenario that wouldn't actually exercise the real caching decision.
