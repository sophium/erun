...
- On current `main` plus this gate's own widening, `TestNoUnboundAppMethods` is clean (whip's desktop control gave `whip` a real reference in `erun-ui/frontend/src`, and `whipAllOrchestratorsNow` is called from `WhipNow` in `erun-ui/whip.go`), but `TestDesktopSurfaceGate` is **not**: 33 API routes across the reviews/builds/comments/merge-queue, releases, tenants, tenant-issuers, plain user CRUD, provision, quota, usage-events, and environment-stop/DNS-01-token families have no reference in either `erun-ui/frontend/src` or `erun-console/src` today. This is a real, pre-existing gap the gate was just taught to see, not a regression from teaching it — most of that surface (erun's own hosted reviews/build/merge-queue system, tenant/role/user administration beyond what `erun-console`'s identity panels already cover) has no console or desktop UI yet. Closing it is tracked as follow-up surface work, not fixed in the same change that taught the gate to detect it.
