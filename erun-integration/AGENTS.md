...
- **A normalizer that silently stops normalizing is worse than one that never existed.** The step-timing table is rendered in real wall-clock order (`erun-common/timing.go`'s `orderedTimingRows` sorts by measured duration), so the only thing that makes it storable in a golden at all is `canonicalizeStepTimingOrder` reordering every level by name. It parses the block by matching each row against `timingLinePattern`, and it ended the block at the first line that did not match — so one unrecognized row shape turned the whole canonicalization off for the tree containing it, *and* reparented the rows after it under the wrong step. The unrecognized shape was a failed step, which appends `" — <error>"` after the duration so the row no longer ends at the bracket, and multi-line errors made a second shape of the same hole. Both went unnoticed for the usual reason a venue-difference bug does: on an idle machine every sibling ties inside production's own 1s noise floor, so the recorded order already *is* name order and the broken normalizer has nothing to correct. It only shows up where timings actually diverge — under a gate's load, inside `erun build` — as a golden diff buried deep in the timing table, which reads as "this scenario fails only in the gate" (erun#2076).
- The rule this leaves behind: **do not gate normalization on a shape assertion that can fail closed and silently.** When a normalizer parses structured output, assert the *property* it exists to provide rather than the pattern that implements it. `TestGoldenTimingBlocksAreOrderInvariant` (`internal/normalize/timingorder_golden_test.go`) is the shape to copy — for every real golden carrying a timing block it reorders the block's siblings and requires canonicalization to reproduce the golden byte for byte, using an indentation-derived parser deliberately independent of `timingLinePattern`, so a row shape the pattern stops recognizing fails loudly here instead of quietly disabling the normalization. It reads goldens outside its own package's compiled inputs, so it depends on `scripts/integration-test.sh` already running this module with `-count=1` (root `AGENTS.md`'s stale-test-cache class).
