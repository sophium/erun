...
- `TestDesktopSurfaceGate` enumerates every capability from three sources — every registered MCP tool (`eruncommon.MCPToolNames()`), every CLI-only command with no MCP equivalent (walked from the real command tree via `cmd.CommandTreeForAudit()`), and every registered HTTP route in `erun-backend-api/internal/routes` (parsed, not imported — that module is a separate Go module the same way `erun-ui` is, so the gate reads its `.go` source with `go/parser` rather than adding a cross-module dependency: every `register(http.MethodX, "path", ...)` call, the `routes.ProtectedRouteRegistrar` convention every route file uses, plus the couple of routes registered directly on the mux for `platform.go`/`invites.go`'s intentionally unauthenticated endpoints) — and fails when a non-exempt one has zero references in the operator surface: the concatenation of `erun-ui/frontend/src` (the desktop app) and `erun-console/src` (the hosted web console), the two committed, human-facing subtrees (their gitignored `wailsjs`/`dist`/`node_modules` siblings don't count, since a generated binding or bundle with no committed UI calling it is not a way in).
...
- A capability opts out of needing an operator entry point via an explicit, reviewable marker instead of a silent omission:
  ...
  - An API route adds its `"METHOD /path"` key to `InternalAPIRoutes` in `erun-backend-api/internal/routes/route_audit.go`, with a comment explaining why — the API-route twin of the two declarations above. `GET /v1/platform` is the one entry today: unauthenticated platform-discovery bootstrap a client fetches before it holds a token, the same infrastructure-only class `erun-backend-api/AGENTS.md`'s Authentication section already carves out for `/healthz`, not an operator-invoked action.
- Every enumerator supplies a `DeclarationHint` naming its own opt-out mechanism, so `Missing.Message()` points at the actual fix for that capability's source instead of a generic list of all three.
...
- `KnownUnsurfacedRoutes` in `erun-backend-api/internal/routes/route_audit.go` holds exactly the 33 routes above, as they stood the day the gate was widened. It is a **record of known gaps, not a design decision** — the opposite claim from `InternalAPIRoutes`, which asserts a route legitimately needs no surface forever. Do not confuse the two lists, and do not add a new route to `KnownUnsurfacedRoutes` to make a fresh gap disappear — it is not a second, softer opt-out.
- A `Capability` sets `KnownGap: true` (with a `BaselineHint`) when its key is in `KnownUnsurfacedRoutes`. `desktopsurface.FindMissingDesktopSurface` skips a `KnownGap` capability exactly like an `AgentFacing` one, which is what lets the widened gate land without fixing all 33 routes in the same change.
...
- Tracking issue: https://github.com/sophium/erun/issues/1497 lists all 33 routes by family and what each needs — a real surface, or a justified move to `InternalAPIRoutes`. `KnownUnsurfacedRoutes`'s own comment points here rather than carrying the issue number itself, per root `AGENTS.md` § "Code Comments"'s "no issue IDs in code comments" rule.
