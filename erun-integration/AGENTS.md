...
- **GitHub REST wire calls with no integration-suite seam**: `postGitHubCommitStatus` (`erun-common/report_commit_status.go`), `listOpenGitHubPullRequests`/`postGitHubIssueComment`/`closeGitHubPullRequest` (`erun-common/close_pull_request.go`), plus `ClosePullRequestHeadMovedError.Error` (only ever called when a real, network-returned head mismatch is found), and `listBypassedRuleSuites`/`getGitHubRuleSuiteDetail`/`githubNextPageLink`/`resolveBypassedPushesForRuleset`/`bypassedRuleForRuleset`/`reconcileAgainstGateRuns`/`finishReconcileBypass` (`erun-common/reconcile_bypass.go`). The harness has no seam to point `githubAPIBaseURL` at a local server the way the chart-registry probe below does, so validation, dry-run tracing, remote-url parsing, and the no-token refusal are exercised end-to-end from the binary in `exec_test.go`'s `report_commit_status_*`/`close_pr_*`/`reconcile_bypass_*` scenarios, while the actual GitHub wire shape, pagination, per-ruleset filtering, gate-run cross-referencing, and failure-response surfacing are covered only by each file's own `erun-common` unit tests (`report_commit_status_test.go`, `close_pull_request_test.go`, `reconcile_bypass_test.go`) against a real `httptest` server. `COVERAGE_THRESHOLD`'s default moved from `75.6` to `75.3` in the commit that added `close_pull_request.go`'s network functions, and from `75.3` to `74.8` in the commit that added `reconcile_bypass.go`'s (measured `75.1%` with the new file's wire calls uncovered), the tracked discussion this section's own rule asks for.
