...
`desktop_surface_test.go` (backed by `internal/desktopsurface/`) enforces root `AGENTS.md` § "Smooth, Seamless, No Dead Ends" failure mode 3: a capability the CLI or MCP can reach but the desktop cannot is a dead end. It runs as an ordinary `go test` in this module, so it is part of `make integration-test`/`make check` with no extra wiring — the same precedent as every other gate in this file. Unlike the per-command golden tests, it is a repo-state structural check, not a `--dry-run` scenario, so it lives at the module root without a matching `testdata/` directory.

- `TestDesktopSurfaceGate` enumerates every capability — every registered MCP tool (`eruncommon.MCPToolNames()`) plus every CLI-only command with no MCP equivalent (walked from the real command tree via `cmd.CommandTreeForAudit()`) — and fails when a non-exempt one has zero references in `erun-ui/frontend/src` (the one committed, human-facing subtree; the gitignored `wailsjs`/`dist`/`node_modules` siblings don't count, since a generated binding with no committed UI calling it is not a way in). The match is a case-insensitive plain substring, deliberately not word-bounded, so a capability named in a camelCase identifier (`WhipButton`) still counts — false positives here only ever make the gate too lenient (a real gap hiding behind a coincidental word match), never too strict, which is the direction that matters: a gate that blocks healthy work gets disabled.
- A capability opts out of needing a desktop entry point via an explicit, reviewable marker instead of a silent omission:
...
- `TestNoUnboundAppMethods` parses every non-test `.go` file directly under `erun-ui` (where `*App` lives) and flags a method on `*App` that is unexported *and* referenced by no other Go identifier in that file set: Wails only binds exported methods, so that combination is a binding location with no binding — the exact shape `whipOrchestratorNow`/`whipAllOrchestratorsNow` shipped in. Export the method or delete it if it is dead.
- Both classifiers are pure functions unit-tested directly in `internal/desktopsurface` against synthetic data (a declared-internal capability, an undeclared one, a camelCase match, an unbound method, a bound one). The wiring test (`desktop_surface_test.go`) supplies the real enumeration and real filesystem reads; keep new coverage of the *logic* in the classifier package, not by growing fixtures against the real repo tree.
- On current `main`, `TestDesktopSurfaceGate` flags only `whip` and `TestNoUnboundAppMethods` flags nothing — whip's desktop control is real, pending work, not a classification bug; see the MCP tool's `AgentFacing: false` (the default) in `mcp_tools.go`.
