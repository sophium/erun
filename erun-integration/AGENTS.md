...
- **Live-network code with no seam**: `releaseArchiveSHA256`/`fetchReleaseArchiveSHA256` GET a hardcoded `https://github.com/...` archive URL with plain `net/http`, and registry version resolution (`registry_versions.go` from the upgrade path) hardcodes `hub.docker.com`/`ghcr.io` base URLs. Covering these needs a URL-base or fetcher seam in production code.
- **Desktop- or MCP-only `erun-common` API**: functions whose only callers live in `erun-ui` or `erun-mcp` (`LogoutCloudProviderAlias`, `ExportCloudProviderCredentials`, `RuntimeDeployVersionSuggestions`, `BuildUpgradePlan`, `FindRootConfigBackupByDate`, `ResolveDoctorTarget`, `ResolveDeploySpecForDockerTarget`) never run inside the instrumented `erun` binary. They are validated by their owning module's suites, not this gate.
