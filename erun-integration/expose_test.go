...
func TestExpose(t *testing.T) {
	...
	// dry_run_backend_service_overrides_the_derived_name is the CLI half of the
	// reported gap: without --backend-service the Ingress targets the derived
	// <tenant>-<service>, and a repo whose own chart names its Service
	// something else gets a hostname that resolves and an Ingress that 503s.
	// The scenario pins both halves -- the public host stays the logical label
	// the caller passed, and only the Ingress backend moves.
	t.Run("dry_run_backend_service_overrides_the_derived_name", func(t *testing.T) {
		setup := env.New(t)
		fixture.SeedTenantEnv(t, setup, "team", "dev")
		fixture.SeedGitRepo(t, setup.Cwd)
		fixture.SeedProjectK8sConfig(t, setup, "platform:\n  basedomain: erunpaas.com\n  env: frs-prod\n  authoritativeip: 203.0.113.10\n")
		result := erun.Run(t, []string{"expose", "team", "dev", "api", "--ip", "203.0.113.10", "--backend-service", "pw-api", "--dry-run"}, erun.RunOptions{Cwd: setup.Cwd, Env: setup.Env()})
		if result.ExitCode != 0 {
			t.Fatalf("exit %d: %s", result.ExitCode, result.Combined)
		}
		golden.Equal(t, "expose/dry_run_backend_service_overrides_the_derived_name", normalize.Apply(result.Combined))
	})
