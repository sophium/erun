...
target/final merge commit, and run one real
`erun build --gated-commit <that same prospective merge commit>` — never
release. Pass `--gated-commit` always: it is what makes the gate an actual
