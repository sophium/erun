...
- **Resolve the target before you arm an irreversible action, not only after it runs.** A relauncher armed against a process that no longer resolves kills nothing, and the relaunch that follows only re-activates what was already there — a silent no-op no different from a restart that did not take. Refuse to arm when the target cannot be named; confirming afterwards is the other half of the check, not a substitute for it.
