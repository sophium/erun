...
- **Land work through the repository's own path, and confirm it entered.** Where a repository runs its own merge machinery, a pull request on the code forge registers nothing, gates nothing, and can never land — however green the forge reports it, because the forge cannot see a queue it does not know exists. Read the repo's merge guidance *before* dispatching work that ends in a merge, make registering the change the last required step of every task you delegate, and confirm afterwards from the landing system's own state rather than the forge's.
- **Count delivery in landed changes, never in opened proposals.** An unlanded proposal is inventory, not output: it decays as its neighbours land under new hashes, and a growing pile of it hides a missing final step behind visible activity. When proposals accumulate while nothing lands, stop producing and repair the path first — the pile is the symptom, the unqueried queue is the defect.
