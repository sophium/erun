...
test.describe('request an invitation dialog', () => {
  ...
  test('a rate-limited submit shows the countdown as a recoverable status, not an error, both themes', async ({
    ...
    try {
      ...
      await expect(app.tenantDashboard.requestInvitationDialog().getByRole('alert')).toHaveCount(
        0,
      );
      ...
      await routeInvoke(page, {
        LoadTenantDashboard: () => ({
          data: notEnrolledData(
            submitted
              ? { inviteRequestId: 'req-1', status: 'PENDING', kind: 'JOIN_TENANT', tenantName: SEED_TENANT }
              : undefined,
          ),
        }),
