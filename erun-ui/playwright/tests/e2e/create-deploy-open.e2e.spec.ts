...
// Opt-in k3d-backed end-to-end coverage. Unlike the default inert
...
// would have caught the create-no-deploy regression end-to-end, which
...
//
// Determinism (no-flaky-tests): every wait is on an observable condition
...
// runs the create handler, which composes the REAL build → push → deploy
...
test.describe('k3d e2e: create → deploy → open (#647)', () => {
  test('create composes a real build→push→deploy and opens the env on k3d', async ({ app }) => {
    ...
    // must build → push → deploy a fresh version.
