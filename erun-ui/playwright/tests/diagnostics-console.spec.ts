...
// Diagnostics console: the bottom panel is a pure viewer with
...
// env (capture is always-on, but the headless run must not depend
...
test.describe('diagnostics console', () => {
  ...
  test('renders the erun trace and UI trace tabs with erun trace active', async ({ app }) => {
    ...
    // The erun-trace toolbar offers Clear (parity with UI trace),
  ...
  test('pane actions live outside the scroll regions and the report button spans tabs', async ({
    ...
    // The toolbars used to render inside the scrollable region, so
  ...
  test('panel surfaces contain no raw ANSI escape sequences', async ({ app }) => {
    // The original regression: the old panel mirrored raw PTY
