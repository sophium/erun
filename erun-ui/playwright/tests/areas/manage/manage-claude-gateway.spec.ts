...
test.describe('per-environment gateway overrides', () => {
  ...
  test('offers leaving the gateway and naming a Secret of its own', async ({ app, page }) => {
    // The catalog is one erun-level list, so without these an environment could
    ...
    await expect(app.manageDialog.claudeGatewaySecretInput()).toBeVisible();
