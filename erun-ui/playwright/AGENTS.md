...
- **No flaky tests.** A spec that passes only sometimes is a defect, not noise — make it deterministic before the PR lands. Never paper over a flake by skipping it, marking it `test.fixme`/`test.skip`, relying on retries to eventually go green, or waving it through as "pre-existing" or "host-environment-flaky". Determinism comes from waiting on observable conditions, never on wall-clock time:
  - Use auto-retrying assertions rather than a one-shot read — `await expect(locator).toBeVisible()` / `.toContainText(...)` / `.toBeEnabled()` / `expect.poll(() => ...)` — and let the assertion absorb a loaded machine. **But know which clock bounds each idiom, because the two do not agree, and that divergence is the class rather than any single number.** `locator.waitFor({ state })`, `page.waitForFunction(...)` and a bare `toPass()` resolve to the **enclosing test's own deadline** (the config's `timeout` — 30s on POSIX, 90s on Windows; no `actionTimeout` is set). `expect(...)` and `expect.poll(...)` do not: with no explicit timeout they resolve to `expect.timeout`, a separate clock the same file sets to 10s on POSIX / 20s on Windows, independent of any `test.setTimeout(...)`. A step that is merely slow therefore reds a test that declared 30s, at 10s, with 20s unspent — and **removing** an explicit timeout from an `expect.poll` is not a widening, because it only lands back on that same 10s default.
  - Point an assertion that has no `waitFor` equivalent — an attribute, a count, a value your `**/__erun_invoke` route handler owns — at the budget its test already declared, with `withTestBudget()` (`fixtures/erunApp.ts`); `tests/areas/manage/` is the reference. Where a locator or a DOM value will serve, prefer the idiom that defers to the test on its own.
  - Do **not** cap a wait below the budget its test declared, in either idiom: a `waitFor({ timeout: 3_000 })` or a `poll(..., { timeout: 15_000 })` inside a 30s test gives up while the test still had room, and a loaded machine reds it. A nested bound belongs inside the enclosing test's budget, never tighter than the clock the assertion would otherwise get.
  - The convergence pass (`ffcb450f`) converted `tests/areas/activity`, `a11y`, `shell` and `diagnostics` onto that shape: a state-changing action is followed by a step that converges on the state it produces (a tab strip's `waitForTab`, a panel's own `waitForOpen()`/`waitForClosed()`), and restore steps converge too so a slow one cannot leak state into the next test in the same worker. `tests/areas/shell/layout.spec.ts` is the reference for a DOM value with no locator to wait on. The pass did not reach every area — a bare `expect(...)` sitting immediately after a click is the defect this describes, not an idiom to copy.
