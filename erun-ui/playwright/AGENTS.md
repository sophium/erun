...
- **Long-lived stubs are registered and reaped.** The `erun` (tab session) and `claude` (orchestrator session) stubs park for the life of the session the desktop spawned for them, because a row reads "running" only while its process is up. The desktop spawns them, so nothing in the harness would otherwise collect one: `writeStubBinary` (POSIX) and `fixtures/winstub` both call `register_stub`/`registerStub` first, appending the pid they park under to `ERUN_PLAYWRIGHT_STUB_REGISTRY` (a file in the isolated root, set by `backendEnv()`), and `fixtures/stubProcesses.ts` reaps that registry on every spec's teardown (`stubReaper`, an automatic fixture in `fixtures/erunApp.ts`) with the worker teardown as the backstop. Keep POSIX and win32 in lockstep, and never park a long-lived stub without registering it — an unregistered stub is unreapable and accumulates for the run.
