...
- Desktop terminal sessions are one per pod per tab ID. Reattach takes over the existing dtach session (`dtach -A <socket> -r <redraw> <launch>`, built by `RemoteAppSessionAttachLines`) rather than mirroring it: the pod session keeps running, and a viewer that loses the takeover only loses its own view. dtach keeps no readable scrollback — a reattached client's screen is cleared, and there is no `screen -X hardcopy`/tmux capture path in the pod — so terminal output you did not see while attached is unrecoverable unless the desktop's own per-tab snapshot still holds it. Do not look for an out-of-band capture route to recover it. Preserve this invariant.
