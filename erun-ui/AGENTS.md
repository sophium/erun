...
- Resolve orchestrator conversations **attached → anchor** through `resolveOrchestratorConversation` (`orchestrator_live_conversation.go`), on restore, start, crash respawn, and restart. There are exactly two sources — `orchestratorConversationAttached` and `orchestratorConversationDerived` — and no third step: a conversation this orchestrator's own session reported being on is **not** adopted automatically, it is offered in the Manage dialog's Conversation section, classified `live`/`stranded`, for the operator to attach deliberately. The deterministic anchor is a first-launch fallback, not proof of the live conversation. Do not collapse the chain to an unconditional derivation: an explicit attachment that stops being honoured strands one conversation per restart, which is the regression the attached step prevents. `orchestrator_live_conversation_test.go` pins the order — `TestRestoreAlwaysResumesTheDerivedAnchorEvenWhenATrackedConversationDiverged` for the absent third step, `TestAttachingAConversationRestartsThereAndSurvivesTheNextLaunch` for the attached step; change the resolution and these change with this record.
