...
- Resolve orchestrator conversations **attached → anchor** through `resolveOrchestratorConversation` on restore, start, crash respawn, and restart. There are exactly two sources (`orchestratorConversationAttached`, `orchestratorConversationDerived`) and no third step: a conversation this orchestrator's own session reported being on is no longer adopted automatically — the anchor is resumed every launch — and is instead offered in the Manage dialog's Conversation section, classified `live`/`stranded`, for the operator to attach deliberately. The deterministic anchor is a first-launch fallback, not proof of the live conversation. Do not collapse the chain to an unconditional derivation: an explicit attachment that stops being honoured strands one conversation per restart, which is the regression the attached step exists to prevent.
