...
- The env-create path composes the deploy itself and gates the open on it (issue #644). `open` is a pure primitive — it no longer deploys (no `--skip-ensure`), so after `==> Initialized` the create handler composes build→push→deploy (`StartDeploySession` → `maybeStartDeployOrchestration` for builds-here envs, an in-shell `erun deploy` for the rest), records the env as pending-open, and opens its tabs only on the matching `environment-deployed` signal (emitted from `finishDeployByTenantEnv` on a succeeded/skipped deploy). Never open tabs directly on create — they would spawn against a runtime that does not exist and fail with an MCP port-forward timeout.
